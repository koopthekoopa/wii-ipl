...
namespace homebutton {
    namespace gui {
            ...
            if (isVisible()) {
                if (contain(x, y)) {
                    ...
                    if (isPointed(point)) {
                        ...
                        offPoint();
                        mpManager->onEvent(getID(), EventHandler::ON_LEFT, data);
                    }
                }
            }
        ...
        bool Manager::update(int point, f32 x, f32 y, u32 trig, u32 hold, u32 release, void* data) {
            ...
            for (u32 i = 0; i < nw4r::ut::List_GetSize(&mComponents); i++) {
                IDToComponent* p = static_cast<IDToComponent*>(nw4r::ut::List_GetNth(&mComponents, i));
            ...
            if (triggerCom) {
                if (trig) {
                    Vec pos;
                    triggerCom->onTrig(trig, pos);
                    onEvent(triggerCom->getID(), EventHandler::ON_TRIG, data);
                }
                if (release) {
                    Vec pos;
                    triggerCom->onTrig(release, pos);
                    onEvent(triggerCom->getID(), EventHandler::ON_RELEASE, data);
                }
            }
