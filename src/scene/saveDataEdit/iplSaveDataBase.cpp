...
namespace ipl {
    namespace scene {
        SavedataBase::SavedataBase(EGG::Heap* heap, nand::LayoutFile* layoutFile, const char* layoutDir, const char* layoutFileName)
        ...
        void SavedataBase::calc() {
            ...
            switch (mState) {
                ...
                case STATE_FADEIN1ST: {
                    on_fadein1st();
                    break;
                }
                case STATE_FADEIN2ND: {
                    on_fadein2nd();
                    break;
                }
                case STATE_FADEOUT: {
        ...
        WAIT_FOR_ANIM_STATE(SavedataBase::on_fadein1st, ANIM_DATA_IN, {
            ...
            mState = STATE_FADEIN2ND;
        });
        WAIT_FOR_ANIM_STATE(SavedataBase::on_fadein2nd, ANIM_SELECT_IN, mState = STATE_IDLE);
        WAIT_FOR_ANIM_STATE(SavedataBase::on_fadeout, ANIM_DATA_OUT, mState = STATE_IDLE);
        WAIT_FOR_ANIM_STATE(SavedataBase::on_change_wii_to_sd, ANIM_SELECT_SD_FLASH, mState = STATE_IDLE);
        WAIT_FOR_ANIM_STATE(SavedataBase::on_change_sd_to_wii, ANIM_SELECT_WII_FLASH, mState = STATE_IDLE);
        ...
        WAIT_FOR_ANIM_STATE(SavedataBase::on_text_fadein, ANIM_ERROR_TEXT_IN, mState = STATE_IDLE);
        WAIT_FOR_ANIM_STATE(SavedataBase::on_text_fadeout, ANIM_ERROR_TEXT_OUT, mState = STATE_IDLE);
    }  // namespace scene
}  // namespace ipl
