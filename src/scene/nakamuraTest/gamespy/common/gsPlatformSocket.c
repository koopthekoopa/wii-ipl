...
HOSTENT * getlocalhost(void)
{
	...
	aAddrs = (IPAddrEntry*)gsimalloc((aNumOfIps * sizeof(IPAddrEntry)));
	memset(aAddrs, 0xBE, (aNumOfIps * sizeof(IPAddrEntry)));

	aAddrsSize = (int)(aNumOfIps * sizeof(IPAddrEntry));
	...
	aNewList = gsimalloc(aNumOfIps * 4);

	for (i = 0; i < aNumOfIps; i++)
	{
		aNewList[i] = aAddrs[i].addr;
	}
	aNewList[i] = NULL;
	aLocalHost.h_addr_list = aNewList;
