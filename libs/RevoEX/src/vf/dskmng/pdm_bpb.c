...
static void VFipdm_bpb_calculate_common_bpb_fields(PDM_BPB* p_bpb) {
    pf_u32 num_data_sectors;
    pf_u16 val;
    pf_u32 fat_max_cluster_count;
    ...
    if (p_bpb->num_clusters < 0xFF5) {
        ...
        fat_max_cluster_count = ((p_bpb->sectors_per_FAT << p_bpb->log2_bytes_per_sector) >> 1U) - 2;
    } else {
        p_bpb->fat_type = PDM_FAT_32;
        fat_max_cluster_count = ((p_bpb->sectors_per_FAT << p_bpb->log2_bytes_per_sector) >> 2U) - 2;
    }
