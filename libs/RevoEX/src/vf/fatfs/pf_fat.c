...
static pf_s32 VFiPFFAT_WriteCluster(PF_FFD* p_ffd, pf_u32 cluster, pf_u32 chain_index, pf_u32 next_cluster, pf_bool use_clstlnk) {
    ...
    if (p_ffd->cluster_link.buffer != PF_NULL && use_clstlnk != PF_FALSE) {
        if (next_cluster == 0) {
...
// DEBUG NON MATCHING
static pf_s32 VFiPFFAT_GetClusterInChain(PF_FFD* p_ffd, pf_u32 chain_index, pf_u32 mode, pf_u32 num_cluster, pf_u32* locate_start,
                                         pf_u32* locate_end) {
    pf_s32 err;
    pf_u32 bad;
    pf_u32 eoc1;
    ...
    *locate_start = *locate_end = -1;
    ...
    if (p_ffd->last_access_cluster.chain_index != 0 && p_ffd->last_access_cluster.chain_index <= chain_index &&
        p_ffd->last_access_cluster.chain_index > (search_index - 1)) {
...
pf_s32 VFiPFFAT_GetBeforeSector(pf_u32* p_befor_sector, PF_VOLUME* p_vol, pf_u32 current_sector) {
    pf_s32 err;
    pf_u32 sector_index;
    pf_u32 current_cluster;
    pf_u32 befor_cluster;

    err = 0;
    if (p_befor_sector == PF_NULL || p_vol == PF_NULL) {
        return 0xA;
    }
...
pf_s32 VFiPFFAT_InitFATRegion(PF_VOLUME* p_vol) {
    ...
    for (sector = p_vol->bpb.num_reserved_sectors; sector < root_dir_sector; sector += num_cache_buff) {
        ...
        err = VFiPFSEC_WriteDataSector(p_vol, p_page->buffer, sector, size, &success_size, 1U, 0U);
        if (err != 0) {
...
pf_s32 VFiPFFAT_WriteValueToSpecifiedCluster(PF_VOLUME* p_vol /* r1+0x8 */, pf_u32 cluster /* r1+0xC */, pf_u32 value /* r1+0x10 */) {
    pf_s32 err;

    err = VFiPFFAT_WriteFATEntry(p_vol, cluster, value);
    if (err != 0) {
        return err;
    }
    return 0;
...
pf_s32 VFiPFFAT_AllocateNumClusters(PF_FFD* p_ffd, pf_u32 allocate_size, pf_u32* p_start_allocated, pf_u32* p_last_allocated,
                                    pf_u32* p_num_allocate_size) {
    pf_u32 clst_size;
    pf_u32 num_cluster;
    pf_u32 chain_index;
    pf_u32 get_num_clusters;
    PF_VOLUME* p_vol;
    pf_s32 err;
    ...
    if ((allocate_size % clst_size) != 0) {
        num_cluster++;
    }
    chain_index = 1;
    err = VFiPFFAT_AllocateChain(p_ffd, num_cluster, chain_index, p_start_allocated, p_last_allocated);
    if (err != 0) {
        return err;
    }
    if ((*p_start_allocated == -1U) || (*p_last_allocated == -1U)) {
