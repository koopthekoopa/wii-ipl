...
static pf_s32 VFiPFCACHE_UpdatePageModified(PF_CACHE_PAGE* p_page, pf_u8* p_sbuf, pf_u8* p_ebuf) {
    p_page->stat |= 2;
    if (p_sbuf != NULL) {
            ...
            if (p_page->sector <= sector && (p_page->sector + p_page->size) >= (sector + num_sector)) {
                ...
                *p_num_success += num_rest_sector;
                ...
                if (p_page->p_mod_sbuf != NULL) {
                    if (p_sbuf >= p_page->p_mod_sbuf) {
                        p_sbuf = NULL;
                    }
                    if (p_page->p_mod_ebuf >= p_ebuf) {
                        p_ebuf = NULL;
                    }
                }
                ...
                *p_num_success += p_page->size;
                VFiPFCACHE_UpdatePageModified(p_page, p_page->buffer, &p_page->buffer[(p_page->size - 1) << p_vol->bpb.log2_bytes_per_sector]);
                ...
                *p_num_success += p_page->size - ((p_page->sector + p_page->size) - (sector + num_sector));
                ...
                VFiPFCACHE_UpdatePageModified(p_page, p_page->buffer, p_ebuf);
            } else if (p_page->sector < sector && (p_page->sector + p_page->size) > sector &&
                       (p_page->sector + p_page->size) <= (sector + num_sector)) {
                VFipf_memcpy(&p_page->buffer[(sector - p_page->sector) << p_vol->bpb.log2_bytes_per_sector], (pf_u8*)p_buf,
                             (p_page->size - (sector - p_page->sector)) << p_vol->bpb.log2_bytes_per_sector);
                ...
                *p_num_success += p_page->size - (sector - p_page->sector);
                ...
                VFiPFCACHE_UpdatePageModified(p_page, p_sbuf, &p_page->buffer[(p_page->size - 1) << p_vol->bpb.log2_bytes_per_sector]);
            }
        }
    } while (p_page != PF_NULL && num_rest_sector != 0);
...
pf_s32 VFiPFCACHE_UpdateModifiedSector(PF_VOLUME* p_vol, PF_CACHE_PAGE* p_page, pf_u32 num_sector) {
    pf_u8* p_sbuf = p_page->p_buf;
    pf_u8* p_ebuf = &p_page->p_buf[(num_sector - 1) << p_vol->bpb.log2_bytes_per_sector];
