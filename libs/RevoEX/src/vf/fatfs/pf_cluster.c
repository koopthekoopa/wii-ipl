...
pf_s32 VFiPFCLUSTER_AdjustCluster(PF_FILE* p_file) {
    ...
    if ((p_file == 0) || ((struct PF_SFD*)p_file->p_sfd == PF_NULL) || ((p_file->stat & 1) == 0) || ((p_file->p_sfd->stat & 1) == 0) ||
    ...
    chain_index = -1U;
    if (p_file->p_sfd->ffd.last_cluster.num_last_cluster != 0) {
    ...
    if (unused_cluster != 0) {
        ...
        if (unused_cluster != sig_eoc) {
            if (file_end_cluster != 0) {
                err = VFiPFFAT_WriteValueToSpecifiedCluster(p_vol, file_end_cluster, sig_eoc);
                if (err != 0) {
                    return err;
                }
                ...
                *p_file->p_sfd->ffd.p_start_cluster = 0;
                ...
                VFiPFFAT_InitHint(p_file->p_sfd->ffd.p_hint);
            ...
            err = VFiPFFAT_FreeChain(&p_file->p_sfd->ffd, unused_cluster, chain_index, -1U);
            if (err != 0) {
                return err;
            }
        }
