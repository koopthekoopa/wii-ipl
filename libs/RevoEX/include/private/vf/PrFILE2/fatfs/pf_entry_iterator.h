...
struct PF_ENT_ITER {
    ...
    PF_FFD ffd;                     // 0x08
    pf_u32 file_sector_index;       // 0x3C
    pf_u32 sector;                  // 0x40
    pf_u16 offset;                  // 0x44
    pf_u16 offset_mask;             // 0x46
    pf_u8 buf[32];                  // 0x48
    pf_u8 log2_entries_per_sector;  // 0x68
};
...
long VFiPFENT_ITER_AllocateEntry(PF_DIR_ENT* p_ent, pf_u8 num_entries, PF_FFD* p_ffd, pf_u32* p_prev_chain, PF_STR* p_filename, pf_u8 attr_required,
                                 pf_u32* p_pos);
