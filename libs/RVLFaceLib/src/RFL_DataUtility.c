...
#define CHECK_BOOL(name, var)                                                                                                                        \
    if (!(var)) {                                                                                                                                    \
        RFLi_REPORT("Invalid " name " data.");                                                                                                       \
        return FALSE;                                                                                                                                \
    }
...
#define CHECK_PADMASK(p, ms, mt)                                                                                                                     \
    if ((lhv->p.rawdata & PAD_MASK(mt, ms)) != (rhv->p.rawdata & PAD_MASK(mt, ms))) {                                                                \
        return FALSE;                                                                                                                                \
    }
...
BOOL RFLiCheckBirthday(u8 month, u8 day) {
    static const u8 scDayMax[12 + 1] = {0, 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
    if (month == 0 && day != 0) {
        return FALSE;
    }
    if (month != 0 && day == 0) {
