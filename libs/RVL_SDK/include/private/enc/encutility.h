...
#define DECLARE_PRIVATE_ENC_FUNCTION_TO_UTF16(srcEnc_)    \
    DECLARE_PRIVATE_ENC_FUNCTION(srcEnc_, u8, Unicode, u16)
#define DECLARE_PRIVATE_ENC_FUNCTION_FROM_UTF16(dstEnc_)    \
    DECLARE_PRIVATE_ENC_FUNCTION(Unicode, u16, dstEnc_, u8)

#define DECLARE_PRIVATE_ENC_FUNCTION_TO_UTF16_WITH_MB_STATE(srcEnc_)        \
    DECLARE_PRIVATE_ENC_FUNCTION_WITH_MB_STATE(srcEnc_, u8, Unicode,    \
                                               u16)
#define DECLARE_PRIVATE_ENC_FUNCTION_FROM_UTF16_WITH_MB_STATE(dstEnc_)        \
    DECLARE_PRIVATE_ENC_FUNCTION_WITH_MB_STATE(Unicode, u16, dstEnc_,    \
                                               u8)
...
#define DEFINE_PUBLIC_ENC_TRAMPOLINE_WITH_MB_STATE_VIA(                    \
    ...
    {                                                                    \
        return ENCiConvertString ## srcVia_ ## To ## dstVia_(            \
            dst, dstSize, src, srcSize, ENC_BR_KEEP, NULL);            \
    }
...
#define DEFINE_PUBLIC_ENC_TRAMPOLINE_FROM_UTF16(dstEnc_)                    \
    DEFINE_PUBLIC_ENC_TRAMPOLINE_VIA(Unicode, u16, Unicode, dstEnc_,    \
                                     u8, dstEnc_)

#define DEFINE_PUBLIC_ENC_TRAMPOLINE_TO_UTF16_VIA(srcEnc_, srcVia_)        \
    DEFINE_PUBLIC_ENC_TRAMPOLINE_VIA(srcEnc_, u8, srcVia_, Unicode,    \
                                     u16, Unicode)

#define DEFINE_PUBLIC_ENC_TRAMPOLINE_TO_UTF16(srcEnc_)                    \
    DEFINE_PUBLIC_ENC_TRAMPOLINE_VIA(srcEnc_, u8, srcEnc_, Unicode,    \
                                     u16, Unicode)

#define DEFINE_PUBLIC_ENC_TRAMPOLINE_WITH_MB_STATE_FROM_UTF16(dstEnc_)            \
    DEFINE_PUBLIC_ENC_TRAMPOLINE_WITH_MB_STATE_VIA(Unicode, u16, Unicode,    \
                                                   dstEnc_, u8, dstEnc_)

#define DEFINE_PUBLIC_ENC_TRAMPOLINE_WITH_MB_STATE_TO_UTF16_VIA(srcEnc_,        \
                                                                srcVia_)        \
    DEFINE_PUBLIC_ENC_TRAMPOLINE_WITH_MB_STATE_VIA(srcEnc_, u8, srcVia_,    \
                                                   Unicode, u16, Unicode)

#define DEFINE_PUBLIC_ENC_TRAMPOLINE_WITH_MB_STATE_TO_UTF16(srcEnc_)            \
    DEFINE_PUBLIC_ENC_TRAMPOLINE_WITH_MB_STATE_VIA(srcEnc_, u8, srcEnc_,    \
                                                   Unicode, u16, Unicode)
...
#define CHECK_PARAMETERS(ret_, dstStream_, dstSize_, dstLimit_, dstValid_,        \
    ...
    {                                                                            \
        *(ret_) = ENCiCheckParameters(                                            \
            (dstStream_) != NULL, dstSize_, dstLimit_, dstValid_,            \
            (srcStream_) != NULL, srcSize_, srcLimit_, srcLimited_);            \
                                                                                \
        if (*(ret_) != ENC_OK)                                            \
            return *(ret_);                                                        \
    } while (0)
