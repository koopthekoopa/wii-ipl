...
{
	BT_HDR				hdr;			// 0x00
	tBTA_DM_SEC_CBACK	*p_sec_cback;	// 0x08
} tBTA_DM_API_ENABLE; // size 0x0c

typedef struct
{
	BT_HDR	hdr;									// 0x00
	char	name[BTA_DM_REMOTE_DEVICE_NAME_LENGTH];	// 0x08 // was BD_NAME_LEN (248)
} tBTA_DM_API_SET_NAME; // size 0x28

typedef struct
{
	BT_HDR			hdr;		// 0x00
	tBTA_DM_DISC	disc_mode;	// 0x08
	tBTA_DM_CONN	conn_mode;	// 0x09
} tBTA_DM_API_SET_VISIBILITY; // size 0x0a

typedef struct
{
	BT_HDR					hdr;		// 0x00
	tBTA_DM_INQ				inq_params;	// 0x08
	/* 2 bytes padding */
	tBTA_SERVICE_MASK		services;	// 0x14
	tBTA_DM_SEARCH_CBACK	*p_cback;	// 0x18
...
typedef struct
{
	...
	/* 2 bytes padding */
	tBTA_SERVICE_MASK		services;	// 0x10
	tBTA_DM_SEARCH_CBACK	*p_cback;	// 0x14
...
typedef struct
{
	BT_HDR			hdr;		// 0x00
	BD_ADDR			bd_addr;	// 0x08
	tBTA_SERVICE_ID	service;	// 0x0e
	tBTA_AUTH_RESP	response;	// 0x0f
} tBTA_DM_API_AUTH_REPLY; // size 0x10

typedef struct
{
	BT_HDR			hdr;	// 0x000
	tBTA_DM_SEARCH	result;	// 0x008
...
{
	BT_HDR	hdr;		// 0x00
	UINT16	sdp_result;	// 0x08
} tBTA_DM_SDP_RESULT; // size 0x0a

typedef struct
{
	BT_HDR					hdr;	// 0x00
	tBTA_SIG_STRENGTH_MASK	mask;	// 0x08
	/* 1 byte padding */
	UINT16					period;	// 0x0a
	BOOLEAN					start;	// 0x0c
	/* 1 byte padding */
...
{
	BT_HDR			hdr;		// 0x00
	tBTM_BL_EVENT	event;		// 0x08
	UINT8			busy_level;	// 0x09
	BOOLEAN			is_new;		// 0x0a
	BD_ADDR			bd_addr;	// 0x0b
	/* 1 byte padding */
...
typedef struct
{
	BT_HDR			hdr;		// 0x00
	BD_ADDR			bd_addr;	// 0x08
	tBTM_PM_STATUS	status;		// 0x0e
	/* 1 byte padding */
	UINT16			value;		// 0x10
	tHCI_STATUS		hci_status;	// 0x12
	/* 1 byte padding */
...
typedef struct
{
	...
	tBTA_DM_CONN_STATE	conn_state;			// 0x06
	tBTA_PREF_ROLES		pref_role;			// 0x07
	BOOLEAN				in_use;				// 0x08
	tBTA_DM_PM_ACTION	pm_mode_attempted;	// 0x09
	tBTA_DM_PM_ACTION	pm_mode_failed;		// 0x0a
} tBTA_DM_PEER_DEVICE; // size 0x0b

typedef struct
{
	tBTA_DM_PEER_DEVICE	peer_device[BTA_DM_NUM_PEER_DEVICE];	// 0x00
	UINT8				count;									// 0x4d
...
typedef struct
{
	...
	tBTA_SYS_ID				id;				// 0x06
	UINT8					app_id;			// 0x07
	tBTA_SYS_CONN_STATUS	state;			// 0x08
} tBTA_DM_SRVCS; // size 0x09

typedef struct
{
	UINT8			count;								// 0x00
	tBTA_DM_SRVCS	conn_srvc[BTA_DM_NUM_CONN_SRVS];	// 0x01
...
typedef struct
{
	...
	tBTA_SYS_ID				id;				// 0x06
	UINT8					app_id;			// 0x07
	tBTA_DM_COMPRESS_STATE	state;			// 0x08
} tBTA_DM_COMPRESS_SRVCS; // size 0x09

typedef struct
{
	TIMER_LIST_ENT	timer;			// 0x00
	BD_ADDR			peer_bdaddr;	// 0x18
	BOOLEAN			in_use;			// 0x1e
	/* 1 byte padding */
...
typedef struct
{
	tBTA_DM_ACTIVE_LINK		device_list;					// 0x000
	/* 2 bytes padding */
	tBTA_DM_SEC_CBACK		*p_sec_cback;					// 0x050
	TIMER_LIST_ENT			signal_strength_timer;			// 0x054
	tBTA_SIG_STRENGTH_MASK	signal_strength_mask;			// 0x06c
	/* 1 byte padding */
	UINT16					state;							// 0x06e
	UINT16					signal_strength_period;			// 0x070
	BOOLEAN					disabling;						// 0x072
	/* 1 byte padding */
	TIMER_LIST_ENT			disable_timer;					// 0x074
	UINT8					num_master_only;				// 0x08c
	/* 3 bytes padding */
	UINT32					wbt_sdp_handle;					// 0x090
	UINT8					wbt_scn;						// 0x094
	UINT8					pm_id;							// 0x095
	/* 2 bytes padding */
	tBTA_PM_TIMER			pm_timer[BTA_DM_NUM_PM_TIMER];	// 0x098
	BD_ADDR					pin_bd_addr;					// 0x0f8
	DEV_CLASS				pin_dev_class;					// 0x0fe
	BOOLEAN					keep_acl_on_shut_down;			// 0x101
	/* 2 bytes padding */
} tBTA_DM_CB; // size 0x104

typedef struct
{
	tBTA_DM_SEARCH_CBACK	*p_search_cback;								// 0x00
	tBTM_INQ_INFO			*p_btm_inq_info;								// 0x04
	tBTA_SERVICE_MASK		services;										// 0x08
	tBTA_SERVICE_MASK		services_to_search;								// 0x0c
	tBTA_SERVICE_MASK		services_found;									// 0x10
	tSDP_DISCOVERY_DB		*p_sdp_db;										// 0x14
	UINT16					state;											// 0x18
	BD_ADDR					peer_bdaddr;									// 0x1a
	char					peer_name[BTA_DM_REMOTE_DEVICE_NAME_LENGTH];	// 0x20 // was BD_NAME_LEN (248)
	TIMER_LIST_ENT			search_timer;									// 0x40
	TIMER_LIST_ENT			rnr_timer;										// 0x58
	UINT8					service_index;									// 0x70
	/* 3 bytes padding */
	tBTA_DM_MSG				*p_search_queue;								// 0x74
	BOOLEAN					wait_disc;										// 0x78
	BOOLEAN					sdp_results;									// 0x79
	/* 2 bytes padding */
...
typedef struct
{
	tBTA_DM_PM_ACTION	power_mode;	// 0x00
	/* 1 byte padding */
	UINT16				timeout;	// 0x02
...
typedef struct
{
	UINT8			allow_mask;							// 0x00
	/* 1 byte padding */
	tBTA_DM_PM_ACTN	actn_tbl[BTA_DM_PM_NUM_EVTS][2];	// 0x02
