...
{
	UINT16	len;					// 0x00
	UINT8	value[MAX_UUID_SIZE];	// 0x02
} tUID_ENT; // size 0x12

typedef struct
{
	UINT16		num_uids;						// 0x000
	tUID_ENT	uuid_entry[MAX_UUIDS_PER_SEQ];	// 0x002
...
typedef struct
{
	UINT16		num_attr;						// 0x00
	tATT_ENT	attr_entry[MAX_ATTR_PER_SEQ];	// 0x02
...
typedef struct
{
	UINT32			record_handle;					// 0x000
	UINT32			free_pad_ptr;					// 0x004
	UINT16			num_attributes;					// 0x008
	/* 2 bytes padding */
	tSDP_ATTRIBUTE	attribute[SDP_MAX_REC_ATTR];	// 0x00c
	UINT8			attr_pad[SDP_MAX_PAD_LEN];		// 0x138
	/* 2 bytes padding */
} tSDP_RECORD; // size 0x298

typedef struct
{
	UINT32		di_primary_handle;			// 0x0000
	BOOLEAN		brcm_di_registered;			// 0x0004
	/* 1 byte padding */
	UINT16		num_records;				// 0x0006
	tSDP_RECORD	record[SDP_MAX_RECORDS];	// 0x0008
} tSDP_DB; // size 0x33e8

typedef struct
{
	tCONN_CB_STATE		con_state;							// 0x000
	tCONN_CB_FLAGS		con_flags;							// 0x001
	...
	tCONN_CB_DISC_STATE	disc_state;							// 0x474
...
typedef struct
{
	tL2CAP_CFG_INFO		l2cap_my_cfg;				// 0x0000
	tCONN_CB			ccb[SDP_MAX_CONNECTIONS];	// 0x003c
	tSDP_DB				server_db;					// 0x121c
	tL2CAP_APPL_INFO	reg_info;					// 0x4604
