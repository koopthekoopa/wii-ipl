...
typedef struct
{
	...
	union
	{
		...
		struct
		{
			UINT8	dlci;			// 0x00
			UINT8	signals;		// 0x01
			UINT8	break_present;	// 0x02
			UINT8	break_duration;	// 0x03
		} msc; // size 0x04
		struct
		{
			UINT8	ea;				// 0x00
		...
		{
			UINT8	dlci;			// 0x00
			UINT8	is_request;		// 0x01
			UINT8	baud_rate;		// 0x02
			UINT8	byte_size;		// 0x03
			UINT8	stop_bits;		// 0x04
			UINT8	parity;			// 0x05
			UINT8	parity_type;	// 0x06
			UINT8	fc_type;		// 0x07
			UINT8	xon_char;		// 0x08
			UINT8	xoff_char;		// 0x09
			UINT16	param_mask;		// 0x0a
		} rpn; // size 0x0c
		struct
		{
			UINT8	dlci;			// 0x00
			UINT8	line_status;	// 0x01
		} rls; // size 0x02
	} u; // 0x08
} MX_FRAME; // size 0x14
...
typedef struct
{
	MX_FRAME			rx_frame;								// 0x00
	tL2CAP_APPL_INFO	reg_info;								// 0x14
	tRFC_MCB			*p_rfc_lcid_mcb[MAX_L2CAP_CHANNELS];	// 0x3c
	BOOLEAN				peer_rx_disabled;						// 0x64
	UINT8				last_mux;								// 0x65
	UINT8				last_port;								// 0x66
	/* 1 byte padding */
} tRFCOMM_CB; // size 0x68

typedef struct
{
	tRFCOMM_CB	rfc;			// 0x000
	tPORT_CB	port;			// 0x068
	UINT8		trace_level;	// 0x414
	/* 3 bytes padding */
} tRFC_CB; // size 0x418
