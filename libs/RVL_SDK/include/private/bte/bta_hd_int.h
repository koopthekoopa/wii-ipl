...
{
	BT_HDR			hdr;									// 0x00
	char			service_name[BTA_HD_APP_NAME_LEN + 1];	// 0x08
	tBTA_HD_CBACK	*p_cback;								// 0x2c
	BD_ADDR			bd_addr;								// 0x30
	UINT8			sec_mask;								// 0x36
	UINT8			app_id;									// 0x37
...
typedef struct
{
	...
	UINT8							event;	// 0x08
	UINT32							data;	// 0x0c
	tHID_DEV_REG_INFO_CBACK_DATA	*pdata;	// 0x10
...
typedef struct
{
	BD_ADDR			peer_addr;	// 0x00
	UINT32			sdp_handle;	// 0x08
	tBTA_HD_CBACK	*p_cback;	// 0x0c
	BUFFER_Q		out_q;		// 0x10
	UINT8			sec_mask;	// 0x1c
	UINT8			state;		// 0x1d
	UINT8			proto;		// 0x1e
...
{
	tQOS_INFO			qos;		// 0x00
	tHID_DEV_SDP_INFO	sdp_info;	// 0x48
	BOOLEAN				use_qos;	// 0xbc
