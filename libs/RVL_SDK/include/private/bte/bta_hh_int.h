...
typedef struct
{
	BT_HDR			hdr;									// 0x00
	UINT8			sec_mask;								// 0x08
	UINT8			service_name[BTA_SERVICE_NAME_LEN + 1];	// 0x09
	/* 3 bytes padding */
...
typedef struct
{
	tHID_DEV_DSCP_INFO	dscp_info;		// 0x00
	BD_ADDR				addr;			// 0x08
	UINT16				attr_mask;		// 0x0e
	UINT16				w4_evt;			// 0x10
	UINT8				index;			// 0x12
...
typedef struct
{
	...
	BOOLEAN	ctrl;								// 0x01
	BOOLEAN	alt;								// 0x02
	BOOLEAN	num_lock;							// 0x03
	BOOLEAN	caps_lock;							// 0x04
	UINT8	last_report[BTA_HH_MAX_RPT_CHARS];	// 0x05
} tBTA_HH_KB_CB; // size 0x0d

typedef struct
{
	tBTA_HH_KB_CB			kb_cb;						// 0x000
	/* 3 bytes padding */
	tBTA_HH_DEV_CB			kdev[BTA_HH_MAX_KNOWN];		// 0x010
	tBTA_HH_DEV_CB			*p_cur;						// 0x210
	UINT8					cb_index[BTA_HH_MAX_KNOWN];	// 0x214
	tBTA_HH_CBACK			*p_cback;					// 0x224
	tSDP_DISCOVERY_DB		*p_disc_db;					// 0x228
