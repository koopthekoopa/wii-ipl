...
{
	struct t_sdp_disc_attr	*p_next_attr;	// 0x00
	UINT16					attr_id;		// 0x04
	UINT16					attr_len_type;	// 0x06
	tSDP_DISC_ATVAL			attr_value;		// 0x08
} tSDP_DISC_ATTR; // size 0x0c

typedef struct t_sdp_disc_rec
{
	tSDP_DISC_ATTR			*p_first_attr;	// 0x00
	struct t_sdp_disc_rec	*p_next_rec;	// 0x04
	UINT32					time_read;		// 0x08
	BD_ADDR					remote_bd_addr;	// 0x0c
	/* 2 bytes padding */
...
typedef struct
{
	...
	UINT16			num_uuid_filters;					// 0x0c
	/* 2 bytes padding */
	tSDP_UUID		uuid_filters[SDP_MAX_UUID_FILTERS];	// 0x10
	UINT16			num_attr_filters;					// 0x4c
	UINT16			attr_filters[SDP_MAX_ATTR_FILTERS];	// 0x4e
	/* 2 bytes padding */
	UINT8			*p_free_mem;						// 0x68
} tSDP_DISCOVERY_DB; // size 0x6c

typedef struct
{
	UINT16	protocol_uuid;						// 0x00
	UINT16	num_params;							// 0x02
	UINT16	params[SDP_MAX_PROTOCOL_PARAMS];	// 0x04
} tSDP_PROTOCOL_ELEM; // size 0x08

typedef struct
{
	UINT16				num_elems;						// 0x00
	tSDP_PROTOCOL_ELEM	list_elem[SDP_MAX_LIST_ELEMS];	// 0x02
...
typedef struct t_sdp_di_record
{
	...
	char	client_executable_url[SDP_MAX_ATTR_LEN];	// 0x09
	char	service_description[SDP_MAX_ATTR_LEN];		// 0x59
	char	documentation_url[SDP_MAX_ATTR_LEN];		// 0xa9
} tSDP_DI_RECORD; // size 0xfa

typedef struct t_sdp_di_get_record
{
	UINT16			spec_id;	// 0x00
	tSDP_DI_RECORD	rec;		// 0x02
