...
void btm_discovery_db_init(void)
{
	...
	btm_cb.btm_disc_vars.uuid_filters[0].uu.uuid16 =
		UUID_SERVCLASS_PUBLIC_BROWSE_GROUP;

	SDP_InitDiscoveryDb(btm_cb.p_btm_ddb, BTM_DISC_DB_SIZE, 1,
	                    btm_cb.btm_disc_vars.uuid_filters, 0, NULL);
}
...
static void btm_disc_compl_cb(UINT16 result)
{
	...
	{
		btm_cb.btm_disc_vars.p_cur_ii =
			BTM_InqDbNext(btm_cb.btm_disc_vars.p_cur_ii);

		if (btm_cb.btm_disc_vars.p_cur_ii
		    && SDP_ServiceSearchRequest(
				btm_cb.btm_disc_vars.p_cur_ii->results.remote_bd_addr,
				btm_cb.p_btm_ddb, &btm_disc_compl_cb))
		{
			return;
		}
	}
...
tBTM_STATUS BTM_StartDiscovery(tBTM_CMPL_CB *p_cmpl_cb, BD_ADDR p_rem_addr)
{
	...
	SDP_InitDiscoveryDb(btm_cb.p_btm_ddb, BTM_DISC_DB_SIZE,
	                    btm_cb.btm_disc_vars.num_uuid_filters,
	                    btm_cb.btm_disc_vars.uuid_filters,
	                    btm_cb.btm_disc_vars.num_attr_filters,
	                    btm_cb.btm_disc_vars.attr_filters);

	if (!SDP_ServiceSearchRequest(p_rem_addr, btm_cb.p_btm_ddb,
	                              &btm_disc_compl_cb))
	{
