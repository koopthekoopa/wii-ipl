...
void sdp_init(void)
{
	...
	if (!BTM_SetSecurityLevel(FALSE, SDP_SERVICE_NAME,
	                          BTM_SEC_SERVICE_SDP_SERVER, SDP_SECURITY_LEVEL,
	                          SDP_PSM, 0, 0))
	{
	...
	if (!BTM_SetSecurityLevel(TRUE, SDP_SERVICE_NAME,
	                          BTM_SEC_SERVICE_SDP_SERVER, SDP_SECURITY_LEVEL,
	                          SDP_PSM, 0, 0))
	{
                            ...
                            UINT8 l2cap_id)
{
	...
	SDP_TRACE(EVENT, "SDP - Rcvd L2CAP conn ind, sent config req, CID 0x%x",
	          p_ccb->connection_id);
}
...
static void sdp_connect_cfm(UINT16 l2cap_cid, UINT16 result)
{
	...
	{
		SDP_TRACE(WARNING, "SDP - Rcvd conn cnf for unknown CID 0x%x",
		          l2cap_cid);
		return;
	}
	...
	if (result == L2CAP_CONN_OK && p_ccb->con_state == SDP_STATE_CONN_SETUP)
	{
		...
		SDP_TRACE(EVENT, "SDP - got conn cnf, sent cfg req, CID: 0x%x",
		          p_ccb->connection_id);
	}
	else
	{
		SDP_TRACE(WARNING, "SDP - Rcvd conn cnf with error: 0x%x  CID 0x%x",
...
static void sdp_config_ind(UINT16 l2cap_cid, tL2CAP_CFG_INFO *p_cfg)
{
	...
	if ((p_ccb = sdpu_find_ccb_by_cid(l2cap_cid)) == NULL)
	{
		SDP_TRACE(WARNING, "SDP - Rcvd L2CAP cfg ind, unknown CID: 0x%x",
		          l2cap_cid);
		return;
	}
	...
	if (p_ccb->con_flags & SDP_FLAGS_OUR_CFG_DONE)
	{
		...
		{
			btu_start_timer(&p_ccb->timer_entry, BTU_TTYPE_SDP,
			                SDP_INACT_TIMEOUT);
		}
	}
}
...
static void sdp_config_cfm(UINT16 l2cap_cid, tL2CAP_CFG_INFO *p_cfg)
{
	...
	SDP_TRACE(EVENT,"SDP - Rcvd cfg cfm, CID: 0x%x  Result: %d", l2cap_cid,
	                 p_cfg->result);
...
static void sdp_disconnect_ind(UINT16 l2cap_cid, BOOLEAN ack_needed)
{
	...
	if ((p_ccb = sdpu_find_ccb_by_cid(l2cap_cid)) == NULL)
	{
		SDP_TRACE(WARNING, "SDP - Rcvd L2CAP disc, unknown CID: 0x%x",
		          l2cap_cid);
		return;
	}
...
static void sdp_data_ind(UINT16 l2cap_cid, BT_HDR *p_msg)
{
	...
	{
		SDP_TRACE(WARNING, "SDP - Rcvd L2CAP data, unknown CID: 0x%x",
		          l2cap_cid);
	}
...
static void sdp_disconnect_cfm(UINT16 l2cap_cid, UINT16 result)
{
	...
	if ((p_ccb = sdpu_find_ccb_by_cid(l2cap_cid)) == NULL)
	{
		SDP_TRACE(WARNING, "SDP - Rcvd L2CAP disc cfm, unknown CID: 0x%x",
		          l2cap_cid);
		return;
	}
...
{
	SDP_TRACE(EVENT, "SDP - CCB timeout in state: %d  CID: 0x%x",
