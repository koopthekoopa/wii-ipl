                            ...
                            tPORT_CALLBACK *p_mgmt_cb)
{
	...
	RFCOMM_TRACE(API,
	             "RFCOMM_CreateConnection() called SCN: %d is_server:%d mtu:%d",
	             scn, is_server, mtu);

	RFCOMM_TRACE(
		API, "RFCOMM_CreateConnection()  BDA: %02x-%02x-%02x-%02x-%02x-%02x",
	...
	*p_handle = 0;
	...
	RFCOMM_TRACE(EVENT, "RFCOMM_CreateConnection dlci:%d signal state:0x%x",
	             dlci, p_port->default_signal_state);
...
int PORT_SetDataCallback(UINT16 port_handle, tPORT_DATA_CALLBACK *p_port_cb)
{
	...
	RFCOMM_TRACE(API, "PORT_SetDataCallback() handle:%d cb 0x%x", port_handle,
	             p_port_cb);
...
int PORT_SetEventMask(UINT16 port_handle, UINT32 mask)
{
	...
	RFCOMM_TRACE(API, "PORT_SetEventMask() handle:%d mask:0x%x", port_handle,
	             mask);
...
int PORT_SetState(UINT16 handle, tPORT_STATE *p_settings)
{
	...
	RFCOMM_TRACE(API, "PORT_SetState() handle:%d FC_TYPE:0x%x", handle,
	             p_settings->fc_type);
...
int PORT_Control(UINT16 handle, UINT8 signal)
{
	...
	RFCOMM_TRACE(
		EVENT, "PORT_Control DTR_DSR : %d, RTS_CTS : %d, RI : %d, DCD : %d",
...
int PORT_FlowControl(UINT16 handle, BOOLEAN enable)
{
	...
	RFCOMM_TRACE(API, "PORT_FlowControl() handle:%d enable: %d", handle,
	             enable);
...
int PORT_GetModemStatus(UINT16 handle, UINT8 *p_signal)
{
	...
	*p_signal = p_port->peer_ctrl.modem_signal;
	...
	RFCOMM_TRACE(API, "PORT_GetModemStatus() handle:%d signal:%x", handle,
	             *p_signal);
...
int PORT_ReadData(UINT16 handle, char *p_data, UINT16 max_len, UINT16 *p_len)
{
	...
	*p_len = 0;
	...
	if (*p_len == 1)
	{
		RFCOMM_TRACE(EVENT, "PORT_ReadData queue:%d returned:%d %x",
		             p_port->rx.queue_size, *p_len, p_data[0]);
	}
	else
	{
		RFCOMM_TRACE(EVENT, "PORT_ReadData queue:%d returned:%d",
		             p_port->rx.queue_size, *p_len);
	}
...
static int port_write(tPORT *p_port, BT_HDR *p_buf)
{
	        ...
	        != (PORT_CTRL_REQ_SENT | PORT_CTRL_IND_RECEIVED))
	{
		...
		{
			RFCOMM_TRACE(WARNING, "PORT_Write: Queue size: %d",
...
int PORT_Write(UINT16 handle, BT_HDR *p_buf)
{
	...
	if (p_port->line_status)
	{
		RFCOMM_TRACE(WARNING, "PORT_Write: Data dropped line_status:0x%x",
		             p_port->line_status);
...
int PORT_WriteData(UINT16 handle, char *p_data, UINT16 max_len, UINT16 *p_len)
{
	...
	*p_len = 0;
	...
	if (!p_port->in_use || p_port->state == PORT_STATE_CLOSED)
	{
		RFCOMM_TRACE(WARNING, "PORT_WriteData() no port state:%d",
		             p_port->state);
		return PORT_NOT_OPENED;
	}
	...
	{
		memcpy((UINT8 *)(p_buf + 1) + p_buf->offset + p_buf->len, p_data,
		       max_len);
