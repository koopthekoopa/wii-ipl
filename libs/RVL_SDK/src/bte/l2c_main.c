...
void l2c_rcv_acl_data(BT_HDR *p_msg)
{
	...
	{
		L2CAP_TRACE(WARNING, "L2CAP - expected pkt start, got: %d",
		            pkt_type);
		GKI_freebuf(p_msg);
		return;
	}
	...
	if ((p_lcb = l2cu_find_lcb_by_handle(handle)) == NULL)
	{
		...
		if (p_msg->layer_specific == 0
		    && rcv_cid == L2CAP_SIGNALLING_CID
		    && (cmd_code == L2CAP_CMD_INFO_REQ
		        || cmd_code == L2CAP_CMD_CONN_REQ) && !btm_sec_is_bonding(handle))
		{
			L2CAP_TRACE(
				WARNING,
			...
			{
				btu_start_timer(&l2cb.rcv_hold_tle, BTU_TTYPE_L2CAP_HOLD,
				                BT_1SEC_TIMEOUT);
			}
	...
	{
		L2CAP_TRACE(WARNING, "L2CAP - bad length in pkt. Exp: %d  Act: %d",
		            l2cap_len, p_msg->len);
...
static void process_l2cap_cmd(tL2C_LCB *p_lcb, UINT8 *p, UINT16 pkt_len)
{
	...
	while (TRUE)
	{
		...
		{
			L2CAP_TRACE(WARNING,
			            "Command len bad  pkt_len: %d  cmd_len: %d  code: %d",
		...
		case L2CAP_CMD_REJECT:
			...
			if (rej_reason == L2CAP_CMD_REJ_MTU_EXCEEDED)
			{
				...
				L2CAP_TRACE(WARNING, "L2CAP - MTU rej Handle: %d MTU: %d",
			...
			if (rej_reason == L2CAP_CMD_REJ_INVALID_CID)
			{
				...
				L2CAP_TRACE(WARNING,
				            "L2CAP - rej with CID invalid, LCID: %d RCID: %d",
				            lcid, rcid);
		...
		case L2CAP_CMD_CONN_REQ:
			...
			{
				L2CAP_TRACE(WARNING,
				            "L2CAP - rcvd conn req for unknown PSM: %d",
				            con_info.psm);
			...
			if ((p_ccb = l2cu_allocate_ccb(p_lcb)) == NULL)
			{
				...
				l2cu_reject_connection(p_lcb, rcid, id,
		...
		case L2CAP_CMD_CONN_RSP:
			...
			{
				L2CAP_TRACE(WARNING,
				            "L2CAP - no CCB for conn rsp, LCID: %d RCID: %d",
			...
			{
				L2CAP_TRACE(WARNING,
				            "L2CAP - con rsp - bad ID. Exp: %d Got: %d",
				            p_ccb->local_id, id);
				break;
			}
		...
		case L2CAP_CMD_CONFIG_REQ:
			...
			if ((p_ccb = l2cu_find_ccb_by_cid(p_lcb, lcid)) != NULL)
			{
				...
				if (cfg_rej)
				{
					l2cu_send_peer_config_rej(
						p_ccb, p_cfg_start,
						(UINT16)(cmd_len - L2CAP_CONFIG_REQ_LEN), cfg_rej_len);
				}
				else
				{
			...
			{
				l2cu_send_peer_cmd_reject(p_lcb, L2CAP_CMD_REJ_INVALID_CID, id,
				                          0, 0);
			}
			break;
		...
		case L2CAP_CMD_CONFIG_RSP:
			...
			if ((p_ccb = l2cu_find_ccb_by_cid(p_lcb, lcid)) != NULL)
			{
				...
				{
					L2CAP_TRACE(WARNING,
					            "L2CAP - cfg rsp - bad ID. Exp: %d Got: %d",
					            p_ccb->local_id, id);
					break;
				}
				...
				{
					l2c_csm_execute(p_ccb, L2CEVT_L2CAP_CONFIG_RSP_NEG,
					                &cfg_info);
				}
			}
			else
			{
				L2CAP_TRACE(WARNING,
				            "L2CAP - rcvd cfg rsp for unknown CID: %d",
				            lcid);
			}
			break;
		...
		case L2CAP_CMD_DISC_REQ:
			...
			if ((p_ccb = l2cu_find_ccb_by_cid(p_lcb, lcid)) != NULL)
			{
				...
				{
					p_ccb->remote_id = id;
					l2c_csm_execute(p_ccb, L2CEVT_L2CAP_DISCONNECT_REQ,
					                &con_info);
				}
			}
			else
		...
		case L2CAP_CMD_DISC_RSP:
			...
			if ((p_ccb = l2cu_find_ccb_by_cid(p_lcb, lcid)) != NULL)
			{
				...
				{
					l2c_csm_execute(p_ccb, L2CEVT_L2CAP_DISCONNECT_RSP,
					                &con_info);
				}
			}
		...
		default:
			...
			l2cu_send_peer_cmd_reject(p_lcb, L2CAP_CMD_REJ_NOT_UNDERSTOOD, id,
			                          0, 0);
			return;
		}
	}
