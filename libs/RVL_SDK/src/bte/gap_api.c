...
UINT16 GAP_FindAddrByName(BD_NAME devname, tGAP_INQ_PARMS *p_inq_parms,
                          tGAP_CALLBACK *p_addr_cb, BD_ADDR bd_addr)
{
	UINT16 status;
	...
	if ((status = gap_find_local_addr_by_name(devname, bd_addr)) != BT_PASS)
	{
		if (!gap_cb.findaddr_cb.in_use)
		{
			...
			if ((status = gap_convert_btm_status(btm_status))
			    == GAP_CMD_INITIATED)
				gap_cb.findaddr_cb.in_use = TRUE;
		}
		else
...
UINT8 GAP_Bond(BD_ADDR bd_addr, UINT8 pin_len, UINT8 *p_pin,
               UINT32 *trusted_mask)
{
...
void GAP_PinRsp(BD_ADDR bd_addr, UINT8 res, UINT8 pin_len, UINT8 *p_pin,
                UINT32 *trusted_mask)
{
...
UINT16 GAP_StartInquiry(tGAP_INQ_PARMS *p_inq_parms,
                        tGAP_CALLBACK *p_results_cb, tGAP_CALLBACK *p_cmpl_cb)
{
	tGAP_INFO *p_cb;
	tBTM_STATUS btm_status;
	UINT16 retval;

	if (p_inq_parms->mode != GAP_GENERAL_INQUIRY
	    && p_inq_parms->mode != GAP_LIMITED_INQUIRY)
	{
		return GAP_ERR_ILL_MODE;
	}

	if (p_inq_parms->duration < GAP_MIN_INQUIRY_LENGTH
                                ...
                                UINT16 max_time, tGAP_CALLBACK *p_results_cb)
{
	...
	if (p_inq_parms->duration < GAP_MIN_INQUIRY_LENGTH
	    ...
	    || min_time > GAP_PER_INQ_MAX_MIN_PERIOD || max_time <= min_time
	    || max_time < GAP_PER_INQ_MIN_MAX_PERIOD
	    || max_time > GAP_PER_INQ_MAX_MAX_PERIOD)
	{
		return GAP_ERR_ILL_INQ_TIME;
	}
...
UINT16 GAP_GetFirstInquiryResult(tGAP_INQ_RESULTS *p_results)
{
	...
	if (gap_cb.cur_inqptr)
	{
		...
		GAP_TRACE(EVENT, "GAP_GetFirstInqResult %02x%02x%02x%02x%02x%02x",
...
UINT16 GAP_GetNextInquiryResult(tGAP_INQ_RESULTS *p_results)
{
	...
	if (gap_cb.cur_inqptr)
	{
		...
		if (gap_cb.cur_inqptr)
		{
			...
			GAP_TRACE(EVENT, "GAP_GetNextInqResult %02x%02x%02x%02x%02x%02x",
			          ptr[0], ptr[1], ptr[2], ptr[3], ptr[4], ptr[5]);
...
UINT16 GAP_ReadLocalDeviceInfo(UINT8 *name, BD_ADDR *addr,
...
UINT16 GAP_GetRemoteDeviceName(BD_ADDR addr, tGAP_CALLBACK *callback)
{
	...
	if ((p_cb = gap_allocate_cb()) != NULL)
	{
		...
		btm_status =
			BTM_ReadRemoteDeviceName(addr, gap_cb.btm_cback[p_cb->index]);
...
void GAP_SetDiscoveryParams(UINT16 num_uuid, tSDP_UUID *p_uuid_list,
