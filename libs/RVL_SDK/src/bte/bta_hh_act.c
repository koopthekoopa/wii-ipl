...
void bta_hh_api_disc_act(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_data)
{
	...
	disc_dat.handle =
		p_data ? (UINT8)p_data->hdr.layer_specific : p_cb->hid_handle;
...
void bta_hh_open_cmpl_act(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_data)
{
	tBTA_HH_CONN conn;
	UINT8 dev_handle =
		p_data ? (UINT8)p_data->hid_cback.hdr.layer_specific : p_cb->hid_handle;
...
void bta_hh_data_act(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_data)
{
	...
	bta_hh_co_data(p_data->hid_cback.hdr.layer_specific, p_rpt, pdata->len,
	               p_cb->mode, p_cb->sub_class, p_cb->app_id);
...
void bta_hh_handsk_act(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_data)
{
	...
	APPL_TRACE(DEBUG, "HANDSHAKE received for: event = %s data= %d",
...
void bta_hh_ctrl_dat_act(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_data)
{
	...
	APPL_TRACE(DEBUG, "Ctrl DATA received w4: event[%s]",
...
void bta_hh_close_act(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_data)
{
	...
	if (reason & (HID_L2CAP_CONN_FAIL | HID_L2CAP_REQ_FAIL | HID_L2CAP_CFG_FAIL)
	    || reason == HID_ERR_AUTH_FAILED || reason == HID_ERR_L2CAP_FAILED
	    || !p_cb->opened)
	{
		conn_dat.handle = p_cb->hid_handle;
		conn_dat.status =
		...
		if (p_cb->hid_handle == BTA_HH_INVALID_HANDLE)
		{
			    ...
			    == HID_SUCCESS)
			{
				...
				bta_hh_add_device_to_list(
					p_cb, dev_handle, p_dev_info->attr_mask, NULL,
					p_dev_info->sub_class, p_dev_info->app_id);
...
void bta_hh_write_dev_act(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_data)
{
	...
	if ((hid_status = HID_HostWriteDev(
			 p_cb->hid_handle, p_data->api_sndcmd.t_type,
                         ...
                         BT_HDR *pdata)
{
	...
	APPL_TRACE(DEBUG, "bta_hh_cback::HID_event [%s]",
