...
UINT8 bta_hh_find_cb(BD_ADDR bda)
{
	...
	{
		if (bdcmp(bda, bta_hh_cb.kdev[xx].addr) == 0
		    && bdcmp(bda, bd_addr_null) != 0)
		{
			APPL_TRACE(DEBUG, "found kdev_cb[%d] hid_handle = %d ", xx,
	...
	APPL_TRACE(DEBUG, "bta_hh_find_cb:: index = %d while max = %d", xx,
	           BTA_HH_MAX_KNOWN);
                               ...
                               UINT8 app_id)
{
	...
	if (p_dscp_info)
	{
		...
		if (p_dscp_info->dl_len
		    && (p_cb->dscp_info.dsc_list = GKI_getbuf(p_dscp_info->dl_len))
		           != NULL)
		{
			p_cb->dscp_info.dl_len = p_dscp_info->dl_len;
			memcpy(p_cb->dscp_info.dsc_list, p_dscp_info->dsc_list,
			       p_dscp_info->dl_len);
		}
	}
}
                           ...
                           UINT16 report_len)
{
	...
	APPL_TRACE(DEBUG, "mice move: x = %d y = %d", p_data->delta_x,
	           p_data->delta_y);
}
...
void bta_hh_trace_dev_db(void)
{
	...
	for (xx = 0; xx < BTA_HH_MAX_KNOWN; ++xx)
	{
		APPL_TRACE(DEBUG, "kdev[%d] in_use[%d]  handle[%d] ", xx,
