...
static tPORT_STATE const default_port_pars =
{
	PORT_BAUD_RATE_9600,
	PORT_8_BITS,
	PORT_ONESTOPBIT,
...
void port_select_mtu(tPORT *p_port)
{
	...
	if (p_port->mtu == 0)
	{
		...
		else if (L2CAP_MTU_SIZE + L2CAP_PKT_OVERHEAD >= packet_size)
		{
			...
			RFCOMM_TRACE(
				DEBUG, "port_select_mtu selected %d based on connection speed",
				p_port->mtu);
		}
		else
		{
	...
	{
		RFCOMM_TRACE(DEBUG, "port_select_mtu application selected %d",
		             p_port->mtu);
	}
...
tPORT *port_find_port(UINT8 dlci, BD_ADDR bd_addr)
{
	...
	for (i = 0; i < MAX_RFC_PORTS; i++)
	{
		...
		if (p_port->in_use && p_port->dlci == dlci
		    && memcmp(p_port->bd_addr, bd_addr, BD_ADDR_LEN) == 0)
		{
			return p_port;
		}
	}
...
UINT32 port_flow_control_user(tPORT *p_port)
{
	...
	BOOLEAN fc = p_port->tx.peer_fc || !p_port->rfc.p_mcb
