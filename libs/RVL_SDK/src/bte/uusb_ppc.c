...
#if UUSB_DBG
# define UUSBDBG(param, ...)	do { ALOGD(param, ##__VA_ARGS__); } while (FALSE)
#else
# define UUSBDBG(param, ...)	do {} while (FALSE)
#endif
...
static void uusb_issue_intr_read(void) NO_INLINE;
...
static void uusb_ReadIntrDataCB(long result, void *p_data)
{
	...
	OSSwitchFiberEx((unsigned long)p_buffer, 0, 0, 0, &bta_ci_hci_msg_handler,
...
static void uusb_issue_bulk_read(void)
{
	...
	// TODO: what is 0x28?
	ret = IUSB_ReadBlkMsgAsync(usb.fd, usb.bulkEndpoint2,
	                           GKI_get_buf_size(p_buf) - 0x28 - p_buf->offset,
	                           p_buffer, uusb_ReadBulkDataCB, p_buf);
...
static void uusb_issue_intr_read(void)
{
	...
	ret = IUSB_ReadIntrMsgAsync(usb.fd, usb.intrEndpoint1,
	                            GKI_get_buf_size(p_buf) - 0x28 - p_buf->offset,
	                            p_buffer, uusb_ReadIntrDataCB, p_buf);
...
static void uusb_WriteCtrlDataCB(long result, void *p_data)
{
	...
	ret = IUSB_WriteCtrlMsgAsync(usb.fd, 0x20, 0, 0, 0, p_buf->len, p_buffer,
...
static void uusb_WriteBulkDataCB(long result, void *p_data)
{
	...
	p_buffer = (UINT8 *)(p_buf + 1) + p_buf->offset;
	ret = IUSB_WriteBlkMsgAsync(usb.fd, usb.bulkEndpoint1, p_buf->len, p_buffer,
	                            uusb_WriteBulkDataCB, p_buf);
