...
static UINT8 *sdpu_build_uuid_seq(UINT8 *p_out, UINT16 num_uuids,
                                  tSDP_UUID *p_uuid_list);
static void sdp_snd_service_search_req(tCONN_CB *p_ccb, UINT8 cont_len,
                                       UINT8 *p_cont);
...
static UINT8 *add_attr(UINT8 *p, tSDP_DISCOVERY_DB *p_db, tSDP_DISC_REC *p_rec,
                       UINT16 attr_id, tSDP_DISC_ATTR *p_parent_attr,
                       UINT8 nest_level);
                                  ...
                                  tSDP_UUID *p_uuid_list)
{
	...
	for (xx = 0; xx < num_uuids; xx++, p_uuid_list++)
	{
		...
		else
		{
			...
			ARRAY_TO_BE_STREAM(p_out, p_uuid_list->uu.uuid128,
			                   p_uuid_list->len);
		}
	}
...
void sdp_disc_server_rsp(tCONN_CB *p_ccb, BT_HDR *p_msg)
{
	...
	if (invalid_pdu)
	{
		SDP_TRACE(WARNING, "SDP - Unexp. PDU: %d in state: %d", rsp_pdu,
		          p_ccb->disc_state);
                                     ...
                                     UINT16 len)
{
	...
	if (p_reply)
	{
		...
		{
			if (!save_attr_seq(p_ccb, p_ccb->rsp_list,
...
static UINT8 *save_attr_seq(tCONN_CB *p_ccb, UINT8 *p, UINT8 *p_msg_end)
{
	...
	while (p < p_seq_end)
	{
		...
		{
			SDP_TRACE(WARNING,"SDP - Bad type: 0x%02x or len: %d in attr_rsp",
			                   type, attr_len);
			return NULL;
		}
