...
void bta_hh_sm_execute(tBTA_HH_DEV_CB *p_cb, UINT16 event, tBTA_HH_DATA *p_data)
{
	...
	if (!p_cb)
	{
		if (bta_hh_cb.p_cback)
		{
			...
			case BTA_HH_API_MAINT_DEV_EVT:
				...
				else
				{
					...
					cback_data.dev_info.handle =
						p_data->api_maintdev.hdr.layer_specific;
				}
			...
			case BTA_HH_API_WRITE_DEV_EVT:
				cback_event =
					(p_data->api_sndcmd.t_type - BTA_HH_FST_BTE_TRANS_EVT)
				    ...
				    || p_data->api_sndcmd.t_type == HID_TRANS_SET_IDLE)
				{
					...
					cback_data.dev_status.handle =
						p_data->api_sndcmd.hdr.layer_specific;
				}
			...
			default:
				APPL_TRACE(ERROR, "wrong device handle: [%d]",
	...
	{
		in_state = p_cb->state;
		APPL_TRACE(EVENT, "bta_hh_sm_execute: State 0x%02x [%s], Event [%s]",
		...
		{
			APPL_TRACE(DEBUG, "HH State Change: [%s] -> [%s] after Event [%s]",
...
BOOLEAN bta_hh_hdl_event(BT_HDR *p_msg)
{
	...
	default:
		...
		APPL_TRACE(DEBUG, "bta_hh_hdl_event:: handle = %d dev_cb[%d] ",
