                            ...
                            UINT8 level, UINT8 scn)
{
	...
	if (!BTM_SetSecurityLevel(bOrig, pName, service, level, BT_PSM_RFCOMM,
	                          BTM_SEC_PROTO_RFCOMM, scn))
	{
		GOEP_TRACE(WARNING, "GOEP:  Security Registration failed for %s",
		           pName);
	...
	*phSDP = SDP_CreateRecord();
	...
	if (SDP_AddServiceClassIdList(*phSDP, num_srv_class, p_service_class))
	{
		...
		if (SDP_AddProtocolList(*phSDP, ARRAY_LENGTH(protoList), protoList))
		{
			if (p_name && *p_name != '\0')
			{
				SDP_AddAttribute(*phSDP, ATTR_ID_SERVICE_NAME,
				                 TEXT_STR_DESC_TYPE, strlen(p_name) + 1,
				                 (UINT8 *)p_name);
			}
	...
	if (status == GOEP_SUCCESS
	    && !SDP_AddUuidSequence(*phSDP, ATTR_ID_BROWSE_GROUP_LIST, 1, &browse))
	{
		status = GOEP_ERROR;
	}
	...
	{
		GOEP_TRACE(EVENT, "GOEP:  Register with SDP returns handle 0x%08x",
		           *phSDP);
	}
