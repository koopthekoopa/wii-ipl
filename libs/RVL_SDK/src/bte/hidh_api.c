...
void hidh_get_str_attr(tSDP_DISC_REC *p_rec, UINT16 attr_id, UINT16 max_len,
                       char *str)
{
...
static void hidh_search_callback(UINT16 sdp_result)
{
	...
	if ((p_nvi->dscp_info.dl_len = SDP_DISC_ATTR_LEN(p_repdesc->attr_len_type))
	    != 0)
	{
	...
	if (((p_attr =
	          SDP_FindAttributeInRec(p_rec, ATTR_ID_HID_RECONNECT_INITIATE))
	     != NULL)
	    && p_attr->attr_value.v.u8)
	{
	...
	if (((p_attr =
	          SDP_FindAttributeInRec(p_rec, ATTR_ID_HID_NORMALLY_CONNECTABLE))
	     != NULL)
	    && p_attr->attr_value.v.u8)
	{
	...
	if ((p_attr =
	          SDP_FindAttributeInRec(p_rec, ATTR_ID_HID_LINK_SUPERVISION_TO))
	     != NULL)
	{
...
tHID_STATUS HID_HostSetSecurityLevel(char *serv_name, UINT8 sec_lvl)
{
	...
	if (!BTM_SetSecurityLevel(TRUE, serv_name, BTM_SEC_SERVICE_HID_INTR,
	                          BTM_SEC_NONE, HID_PSM_INTERRUPT,
	                          BTM_SEC_PROTO_HID, 0))
	{
