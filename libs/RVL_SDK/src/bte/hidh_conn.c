                                   ...
                                   UINT16 psm, UINT8 l2cap_id)
{
	...
	HIDH_TRACE(EVENT, "HID - Rcvd L2CAP conn ind, PSM: 0x%04x  CID 0x%x", psm,
	                  l2cap_cid);
	...
	{
		L2CA_ConnectRsp(bd_addr, l2cap_id, l2cap_cid, L2CAP_CONN_SECURITY_BLOCK,
		                0);
		return;
	}
	...
	if (!bAccept)
	{
		L2CA_ConnectRsp(bd_addr, l2cap_id, l2cap_cid, L2CAP_CONN_NO_RESOURCES,
		                L2CAP_CONN_OK);
		return;
	}
...
void hidh_proc_repage_timeout(TIMER_LIST_ENT *p_tle)
{
	...
	(*hh_cb.callback)((UINT32)p_tle->param, HID_HDEV_EVT_RETRYING,
	                  hh_cb.devices[(UINT32)p_tle->param].conn_tries, NULL);
}
...
void hidh_sec_check_complete_orig(BD_ADDR bd_addr, void *p_ref_data, UINT8 res)
{
	...
	if (res == BTM_SUCCESS && p_dev->conn.conn_state == HID_CONN_STATE_SECURITY)
	{
		...
		if ((p_dev->conn.intr_cid = L2CA_ConnectReq(
...
static void hidh_l2cif_connect_cfm(UINT16 l2cap_cid, UINT16 result)
{
	...
	{
		HIDH_TRACE(WARNING, "HID - Rcvd unexpected conn cnf, CID 0x%x ",
		           l2cap_cid);
		return;
	}
	...
	HIDH_TRACE(EVENT, "HID - got CTRL conn cnf, sent cfg req, CID: 0x%x",
	           l2cap_cid);
}
...
static void hidh_l2cif_config_ind(UINT16 l2cap_cid, tL2CAP_CFG_INFO *p_cfg)
{
	...
	if (!p_hcon)
	{
		HIDH_TRACE(WARNING, "HID - Rcvd L2CAP cfg ind, unknown CID: 0x%x",
		           l2cap_cid);
		return;
	}
...
static void hidh_l2cif_config_cfm(UINT16 l2cap_cid, tL2CAP_CFG_INFO *p_cfg)
{
	...
	HIDH_TRACE(EVENT, "HID - Rcvd cfg cfm, CID: 0x%x  Result: %d", l2cap_cid,
	           p_cfg->result);
...
static void hidh_l2cif_disconnect_ind(UINT16 l2cap_cid, BOOLEAN ack_needed)
{
	...
	if (p_hcon == NULL)
	{
		HIDH_TRACE(WARNING, "HID - Rcvd L2CAP disc, unknown CID: 0x%x",
		           l2cap_cid);
		return;
	}
	...
	if (!p_hcon->ctrl_cid && !p_hcon->intr_cid)
	{
		...
		(*hh_cb.callback)(dhandle, HID_HDEV_EVT_CLOSE, hid_close_evt_reason,
		                  NULL);
	}
}
...
static void hidh_l2cif_disconnect_cfm(UINT16 l2cap_cid, UINT16 result)
{
	...
	if (!p_hcon)
	{
		HIDH_TRACE(WARNING, "HID - Rcvd L2CAP disc cfm, unknown CID: 0x%x",
		           l2cap_cid);
		return;
	}
	...
	if (!p_hcon->ctrl_cid && !p_hcon->intr_cid)
	{
		hh_cb.devices[dhandle].state = HID_DEV_NO_CONN;
		p_hcon->conn_state = HID_CONN_STATE_UNUSED;
		(*hh_cb.callback)(dhandle, HID_HDEV_EVT_CLOSE, p_hcon->disc_reason,
		                  NULL);
	}
}
...
static void hidh_l2cif_cong_ind(UINT16 l2cap_cid, BOOLEAN congested)
{
	...
	HIDH_TRACE(EVENT, "HID - Rcvd L2CAP congestion status, CID: 0x%x  Cong: %d",
	           l2cap_cid, congested);
...
static void hidh_l2cif_data_ind(UINT16 l2cap_cid, BT_HDR *p_msg)
{
	...
	if (!p_hcon)
	{
		HIDH_TRACE(WARNING, "HID - Rcvd L2CAP data, unknown CID: 0x%x",
		           l2cap_cid);
		GKI_freebuf(p_msg);
		return;
	}
...
static void hidh_conn_retry(UINT8 dhandle)
{
	...
	btu_start_timer(&p_dev->conn.timer_entry, BTU_TTYPE_HID_HOST_REPAGE_TO,
	                HID_HOST_REPAGE_WIN);
}
