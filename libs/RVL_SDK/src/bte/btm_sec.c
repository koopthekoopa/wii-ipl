...
void BTM_SetPinType(UINT8 pin_type, PIN_CODE pin_code, UINT8 pin_code_len)
{
	BTM_TRACE(
		...
		pin_type, pin_code, pin_code_len);
                             ...
                             UINT32 mx_proto_id, UINT32 mx_chan_id)
{
	...
	for (index = 0; index < BTM_SEC_MAX_SERVICE_RECORDS; index++, p_srec++)
	{
		if (p_srec->security_flags & BTM_SEC_IN_USE)
		{
			...
			if (strncmp(p_name, (char *)p_srec->orig_service_name,
	...
	if (record_allocated)
	{
		...
		if (is_originator)
		{
			...
			BCM_STRNCPY_S((char *)p_srec->orig_service_name,
			              sizeof p_srec->orig_service_name, p_name,
			              BTM_SEC_SERVICE_NAME_LEN);
		}
		else
		{
			...
			BCM_STRNCPY_S((char *)p_srec->term_service_name,
			              sizeof p_srec->term_service_name, p_name,
			              BTM_SEC_SERVICE_NAME_LEN);
		}
		...
		BTM_TRACE(API,
		          ...
		          "%d, chan_id %d",
		          index, service_id, is_originator, psm, mx_proto_id,
		          mx_chan_id);
...
void BTM_DeviceAuthorized(BD_ADDR bd_addr, UINT8 res, UINT32 *trusted_mask)
{
	...
	BTM_TRACE(EVENT, "Security Manager: authorized status:%d State:%d", res,
	          p_dev_rec ? p_dev_rec->sec_state : 0);
                        ...
                        UINT32 *trusted_mask)
{
	...
	BTM_TRACE(API, "BTM_SecBond BDA: %02x:%02x:%02x:%02x:%02x:%02x", bd_addr[0],
	...
	{
		result = btsnd_hcic_create_conn(
			bd_addr, 24, p_inq_info->results.page_scan_rep_mode,
			p_inq_info->results.page_scan_mode,
			p_inq_info->results.clock_offset | 0x8000, allow_switch);
	}
	else
	{
		result = btsnd_hcic_create_conn(bd_addr, 24, 1, 0,
		                                p_dev_rec->clock_offset, allow_switch);
	}
                              ...
                              void *p_ref_data)
{
	...
	if ((p_dev_rec->sec_flags & (BTM_SEC_AUTHENTICATED | BTM_SEC_ENCRYPTED))
                                     ...
                                     tBTM_SEC_CALLBACK *p_callback)
{
	...
	if (!p_dev_rec)
	{
		...
		{
			BTM_TRACE(WARNING,
			          "Security Manager: l2cap_access_req PSM:%d no resources",
			          psm);
	...
	BTM_TRACE(EVENT,
	          ...
	          "Flags:0x%x Required:0x%x",
	          psm, handle, p_dev_rec->sec_state, p_dev_rec->sec_flags,
	          security_required);
	...
	if (!is_originator && security_required & 1)
	{
		...
		BTM_TRACE(EVENT, "Security Manager: trusted:0x%04x%04x Flags:0x%x",
                               ...
                               void *p_ref_data)
{
	...
	if (!p_serv_rec)
	{
		...
		BTM_TRACE(
			ERROR,
			"Security Manager: MX service not found PSM:%d Proto:%d SCN:%d",
...
void btm_sec_abort_access_req(BD_ADDR bd_addr)
{
	...
	{
		(*btm_cb.api.p_abort_callback)(bd_addr, p_dev_rec->dev_class,
                                       ...
                                       BD_NAME_PTR p_bd_name, UINT8 status)
{
	...
	BTM_TRACE(EVENT, "Security Manager: rmt_name_complete status:%d State:%d",
		...
		for (i = 0; i < BTM_SEC_MAX_RMT_NAME_CALLBACKS; ++i)
		{
			if (btm_cb.p_rmt_name_callback[i])
			{
				(*btm_cb.p_rmt_name_callback[i])(
					p_bd_addr, p_dev_rec->dev_class, p_dev_rec->sec_bd_name);
			}
		}
	}
	...
	if (status != 0)
	{
		if (p_dev_rec->p_callback)
		{
			(*p_dev_rec->p_callback)(p_dev_rec->bd_addr, p_dev_rec->p_ref_data,
			                         10);
...
void btm_sec_auth_complete(UINT16 handle, UINT8 status)
{
	...
	BTM_TRACE(EVENT, "Security Manager: auth_complete status:%d State:%d",
	          status, p_dev_rec ? p_dev_rec->sec_state : 0);
	...
	if (p_dev_rec && p_dev_rec->security_required & 0x40
	    && !btm_cb.bond_created_connection)
	{
		p_dev_rec->security_required &= ~0x50;
...
void btm_sec_mkey_comp_event(UINT16 handle, UINT8 status, UINT8 key_flg)
{
	...
	BTM_TRACE(EVENT, "Security Manager: mkey comp status:%d State:%d", status,
	          (p_dev_rec) ? p_dev_rec->sec_state : 0);
...
void btm_sec_encrypt_change(UINT16 handle, UINT8 status, UINT8 encr_enable)
{
	...
	BTM_TRACE(EVENT, "Security Manager: encrypt_change status:%d State:%d",
	          status, p_dev_rec ? p_dev_rec->sec_state : 0);
	...
	if (status == HCI_ERR_LMP_ERR_TRANS_COLLISION)
	{
		...
		btm_cb.sec_collision_tle.param =
			(TIMER_PARAM_TYPE)&btm_sec_collision_timeout;
...
void btm_sec_connected(BD_ADDR bda, UINT16 handle, UINT8 status, UINT8 enc_mode)
{
	...
	BTM_TRACE(
		EVENT,
		"Security Manager: btm_sec_connected handle:%d status:%d enc_mode:%d",
	...
	if (status != 0)
	{
		if (0
			...
			|| status == 0x17
			|| status == 0x22
			|| p_dev_rec->security_required & 0x40
		)
		{
			p_dev_rec->security_required &= ~0x50;
			p_dev_rec->sec_flags &= ~0x10;
		...
		else
		{
			if (p_dev_rec->p_callback)
			{
				...
				{
					(*p_dev_rec->p_callback)(p_dev_rec->bd_addr,
					                         p_dev_rec->p_ref_data, 8);
				}
				else
				{
					(*p_dev_rec->p_callback)(p_dev_rec->bd_addr,
					                         p_dev_rec->p_ref_data, 10);
				}
			}
		}
	    ...
	    && btm_cb.api.p_auth_complete_callback)
	{
		if (reason == 23)
		{
			(*btm_cb.api.p_auth_complete_callback)(p_dev_rec->bd_addr,
			                                       p_dev_rec->dev_class,
			                                       p_dev_rec->sec_bd_name, 23);
		}
		else
		{
			(*btm_cb.api.p_auth_complete_callback)(p_dev_rec->bd_addr,
			                                       p_dev_rec->dev_class,
			                                       p_dev_rec->sec_bd_name, 5);
		}
	}
                                   ...
                                   UINT8 key_type)
{
	...
	BTM_TRACE(
		EVENT,
		"btm_sec_link_key_notification()  BDA: %02x:%02x:%02x:%02x:%02x:%02x",
...
void btm_sec_link_key_request(BD_ADDR_PTR p_bda)
{
	...
	if (!(p_dev_rec->security_required & 0x40)
	    && btm_cb.api.p_link_key_req_callback)
	{
		if (p_dev_rec)
		{
			if (!(*btm_cb.api.p_link_key_req_callback)(p_bda,
			                                           p_dev_rec->link_key))
			{
				btsnd_hcic_link_key_req_reply(p_bda, p_dev_rec->link_key);
				return;
...
void btm_sec_pin_code_request(BD_ADDR_PTR p_bda)
{
	...
	BTM_TRACE(EVENT,
	          "btm_sec_pin_code_request()  BDA: %02x:%02x:%02x:%02x:%02x:%02x",
	          p_bda[0], p_bda[1], p_bda[2], p_bda[3], p_bda[4], p_bda[5]);
	...
	if (!p_cb->pairing_disabled && p_cb->cfg.pin_type == 1)
	{
		...
		{
			/* explicitly post-increment */
			for (p_dev_rec = btm_cb.sec_dev_rec; i < BTM_SEC_MAX_DEVICE_RECORDS;
			     i++, p_dev_rec++)
			{
				if (!(p_dev_rec->sec_flags & 0x80))
					break;
	...
	if (p_dev_rec)
	{
		...
		if (p_dev_rec->security_required & 0x40)
		{
			...
			btsnd_hcic_pin_code_req_reply(p_bda, p_cb->pin_code_len,
			                              p_cb->pin_code);
			...
			if (btm_cb.security_mode_changed)
			{
				...
				BTM_TRACE(DEBUG,
				          "btm_sec_pin_code_request: Authen Enable -> %d",
		...
		else
		{
			...
			else
			{
				...
				p_cb->sec_pin_code_req_tle.param =
					(TIMER_PARAM_TYPE)&btm_sec_pin_code_request_timeout;
...
static tBTM_STATUS btm_sec_execute_procedure(tBTM_SEC_DEV_REC *p_dev_rec)
{
	            ...
	            && (p_dev_rec->security_required & BTM_SEC_IN_AUTHORIZE))))
	{
		if (!btm_sec_are_all_trusted(p_dev_rec->trusted_mask)
		    ...
		    && !BTM_SEC_IS_SERVICE_TRUSTED(
				p_dev_rec->trusted_mask, p_dev_rec->p_cur_service->service_id))
		{
	...
	BTM_TRACE(EVENT, "Security Manager: trusted:0x%04x%04x",
	          p_dev_rec->trusted_mask[1], p_dev_rec->trusted_mask[0]);
...
static BOOLEAN btm_sec_start_get_name(tBTM_SEC_DEV_REC *p_dev_rec)
{
	...
	if (btm_initiate_rem_name(p_dev_rec->bd_addr, NULL, BTM_RMT_NAME_SEC, 30,
	                          NULL)
	    != BTM_CMD_STARTED)
	{
	...
	for (i = 0; i < BTM_SEC_MAX_SERVICE_RECORDS; i++, p_serv_rec++)
	{
		if (p_serv_rec->security_flags & BTM_SEC_IN_USE
		    ...
		    && ((is_originator && p_serv_rec->orig_mx_chan_id == mx_chan_id)
		        || (!is_originator
		            && p_serv_rec->term_mx_chan_id == mx_chan_id)))
		{
			return p_serv_rec;
		}
	}
...
static void btm_sec_collision_timeout(TIMER_LIST_ENT *p_tle)
{
	...
	if (btm_cb.p_collided_dev_rec->p_callback)
	{
		...
		(*btm_cb.p_collided_dev_rec->p_callback)(
			btm_cb.p_collided_dev_rec->bd_addr,
...
static void btm_restore_mode(void)
{
	...
	BTM_TRACE(DEBUG, "btm_restore_mode: Authen Enable -> %d",
