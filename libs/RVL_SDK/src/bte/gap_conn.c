...
static void gap_connect_ind(BD_ADDR bd_addr, UINT16 l2cap_cid, UINT16 psm,
                            UINT8 l2cap_id);
                    ...
                    UINT8 security, tGAP_CONN_CALLBACK *p_cb)
{
	...
	if (!BTM_SetSecurityLevel(!is_server, p_serv_name, service_id, security,
	                          psm, 0, 0))
	{
...
UINT16 GAP_ConnReadData(UINT16 gap_handle, UINT8 *p_data, UINT16 max_len,
                        UINT16 *p_len)
{
	tGAP_CCB *p_ccb = gap_find_ccb_by_handle(gap_handle);
	BT_HDR *p_buf;
...
UINT16 GAP_ConnWriteData(UINT16 gap_handle, UINT8 *p_data, UINT16 max_len,
                         UINT16 *p_len)
{
	tGAP_CCB *p_ccb = gap_find_ccb_by_handle(gap_handle);
	BT_HDR *p_buf;
                            ...
                            UINT8 l2cap_id)
{
	     ...
	     xx++, p_ccb++)
	{
		if (p_ccb->con_state == GAP_CCB_STATE_LISTENING && p_ccb->psm == psm
		    && (!p_ccb->rem_addr_specified
		        || memcmp(bd_addr, p_ccb->rem_dev_address, BD_ADDR_LEN) == 0))
		{
			break;
		}
	}
	...
	GAP_TRACE(EVENT, "GAP_CONN - Rcvd L2CAP conn ind, CID: 0x%x",
	          p_ccb->connection_id);
...
static void gap_congestion_ind(UINT16 lcid, BOOLEAN is_congested)
{
	...
	GAP_TRACE(EVENT, "GAP_CONN - Rcvd L2CAP Is Congested (%d), CID: 0x%x",
	          is_congested, lcid);
