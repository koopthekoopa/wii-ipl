                             ...
                             tBTM_CMPL_CB *p_cmpl_cb)
{
	...
	BTM_TRACE(API, "BTM_StartInquiry: mode: %d, dur: %d, rsps: %d, flt: %d",
	          p_inqparms->mode, p_inqparms->duration, p_inqparms->max_resps,
...
tBTM_INQ_INFO *BTM_InqDbRead(BD_ADDR p_bda)
{
	...
	BTM_TRACE(API, "BTM_InqDbRead: bd addr [%02x%02x%02x%02x%02x%02x]",
...
tBTM_STATUS BTM_ClearInqDb(BD_ADDR p_bda)
{
	...
	if (p_inq->inq_active != BTM_INQUIRY_INACTIVE || p_inq->remname_active
...
UINT8 BTM_ReadNumInqDbEntries(void)
{
	...
	/* explicitly post-increment */
	for (num_entries = 0, num_results = 0; num_entries < BTM_INQ_DB_SIZE;
	     num_entries++, p_ent++)
	{
		if (p_ent->in_use)
			++num_results;
...
static BOOLEAN btm_inq_find_bdaddr(BD_ADDR p_bda)
{
	...
	{
		if (memcmp(p_db->bd_addr, p_bda, BD_ADDR_LEN) == 0
	...
	tBTM_INQ_FILT_COND_TYPE filter_cond_type, tBTM_INQ_FILT_COND *p_filt_cond)
{
	...
	if (p_buf)
	{
		...
		{
		case BTM_FILTER_COND_DEVICE_CLASS:
			memcpy(condition_buf, p_filt_cond->cod_cond.dev_class,
			       DEV_CLASS_LEN);
...
static void btm_initiate_inquiry(tBTM_INQUIRY_VAR_ST *p_inq)
{
	...
	p_inqparms->max_resps = p_inqparms->max_resps <= BTM_INQ_DB_SIZE
	...
	{
		if (!btsnd_hcic_per_inq_mode(p_inq->per_max_delay, p_inq->per_min_delay,
                                  ...
                                  UINT32 timeout, tBTM_CMPL_CB *p_cb)
{
	...
	if (!(p_inq->remname_active & origin))
	{
		...
		if (send_to_controller)
		{
			...
			btu_start_timer(&p_inq->rmt_name_timer_ent, BTU_TTYPE_BTM_RMT_NAME,
			                timeout);
			...
			{
				cmd_ok = btsnd_hcic_rmt_name_req(
					remote_bda, HCI_PAGE_SCAN_REP_MODE_R1,
