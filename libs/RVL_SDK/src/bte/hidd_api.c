...
UINT32 HID_DevSetSDPRecord(tHID_DEV_SDP_INFO *p_sdp_info)
{
	...
	{
		SDP_AddAttribute(sdp_handle, 0x100, 4,
		                 (UINT8)(strlen(p_sdp_info->svc_name) + 1),
		                 (UINT8 *)p_sdp_info->svc_name);
	}

	if (p_sdp_info->svc_descr[0] != '\0')
	{
		SDP_AddAttribute(sdp_handle, 0x101, 4,
		                 (UINT8)(strlen(p_sdp_info->svc_descr) + 1),
		                 (UINT8 *)p_sdp_info->svc_descr);
	}

	if (p_sdp_info->prov_name[0] != '\0')
	{
		SDP_AddAttribute(sdp_handle, 0x102, 4,
		                 (UINT8)(strlen(p_sdp_info->prov_name) + 1),
		                 (UINT8 *)p_sdp_info->prov_name);
	}
