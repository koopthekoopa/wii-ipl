...
static void rfc_port_sm_term_wait_sec_check(tPORT *p_port, UINT16 event,
                                            void *p_data);
static void rfc_port_sm_orig_wait_sec_check(tPORT *p_port, UINT16 event,
                                            void *p_data);
...
static void rfc_port_sm_state_closed(tPORT *p_port, UINT16 event, void *p_data)
{
	...
	case RFC_EVENT_OPEN:
		...
		btm_sec_mx_access_request(p_port->rfc.p_mcb->bd_addr, BT_PSM_RFCOMM,
		                          TRUE, BTM_SEC_PROTO_RFCOMM, p_port->dlci / 2,
	...
	case RFC_EVENT_SABME:
		...
		btm_sec_mx_access_request(p_port->rfc.p_mcb->bd_addr, BT_PSM_RFCOMM,
		                          FALSE, BTM_SEC_PROTO_RFCOMM, p_port->dlci / 2,
		                          &rfc_sec_check_complete, p_port);
		return;
	...
	case RFC_EVENT_TIMEOUT:
		...
		RFCOMM_TRACE(ERROR, "Port error state %d event %d", p_port->rfc.state,
		             event);
		return;
	}
                                            ...
                                            void *p_data)
{
	...
	RFCOMM_TRACE(WARNING, "Port state term_wait_sec_check Event ignored %d",
	             event);
}
                                            ...
                                            void *p_data)
{
	...
	RFCOMM_TRACE(WARNING, "Port state orig_wait_sec_check Event ignored %d",
	             event);
}
