...
BOOLEAN l2c_link_hci_conn_comp(UINT8 status, UINT16 handle, BD_ADDR p_bda)
{
	...
	if (p_lcb->link_state != LST_CONNECTING)
	{
		L2CAP_TRACE(ERROR, "L2CAP got conn_comp in bad state: %d  status: 0x%d",
...
BOOLEAN l2c_link_send_to_lower(tL2C_LCB *p_lcb, BT_HDR *p_buf)
{
	...
	{
		num_segs = (p_buf->len - HCI_DATA_PREAMBLE_SIZE
		            + btu_cb.hcit_acl_data_size - 1)
                              ...
                              BT_HDR *p_buf)
{
	...
	if (p_buf)
	{
		if (p_lcb->link_xmit_data_q.count >= p_lcb->link_cong_start
		    && !p_lcb->cong_sent)
		{
			for (p_ccb = p_lcb->p_first_ccb; p_ccb; p_ccb = p_ccb->p_next_ccb)
			{
				...
				if (p_ccb->p_rcb->api.pL2CA_CongestionStatus_Cb)
				{
					...
					(*p_ccb->p_rcb->api.pL2CA_CongestionStatus_Cb)(
						p_ccb->local_cid, TRUE);
				}
			}
	...
	if (BTM_ReadPowerMode(p_lcb->remote_bd_addr, &mode) == BTM_SUCCESS
	    && mode == BTM_ACL_MODE_PARK && p_lcb->link_xmit_data_q.count != 0)
	{
...
void l2c_link_adjust_allocation(void)
{
	...
	for (yy = 0, p_lcb = l2cb.lcb_pool; yy < MAX_L2CAP_LINKS; yy++, p_lcb++)
	{
		...
		L2CAP_TRACE(DEBUG,
		            ...
		            "End:%d Discard:%d",
		            yy, p_lcb->acl_priority, p_lcb->link_xmit_window,
		            p_lcb->link_cong_start, p_lcb->link_cong_end,
		            p_lcb->link_cong_discard);
	}
}
...
BT_HDR *l2cap_link_chk_pkt_start(BT_HDR *p_cur_buf)
{
	...
	else
	{
		if (p_lcb->p_hcit_rcv_acl)
		{
			UINT8 *p_f = (UINT8 *)(p_lcb->p_hcit_rcv_acl + 1)
			           + p_lcb->p_hcit_rcv_acl->offset + 2;
			UINT16 total_len;
			...
			if (total_len + hci_len <= L2CAP_MTU_SIZE + HCI_DATA_PREAMBLE_SIZE)
			{
				...
				if (p_cur_buf->len > HCI_DATA_PREAMBLE_SIZE)
				{
					...
					UINT8 *p1 =
						(UINT8 *)(result + 1) + result->offset + result->len;
...
BOOLEAN l2cap_link_chk_pkt_end(void)
{
	...
	if (((p_lcb = l2cb.p_cur_hcit_lcb) == NULL)
	    || ((p_buf = p_lcb->p_hcit_rcv_acl) == NULL))
	{
		return TRUE;
	}
