...
static void hidd_l2cif_connect_ind(BD_ADDR bd_addr, UINT16 l2cap_cid,
                                   UINT16 psm, UINT8 l2cap_id);
static void hidd_l2cif_connect_cfm(UINT16 l2cap_cid, UINT16 result);
static void hidd_l2cif_config_ind(UINT16 l2cap_cid,
                                  tL2CAP_CFG_INFO *p_cfg);
static void hidd_l2cif_config_cfm(UINT16 l2cap_cid,
                                  tL2CAP_CFG_INFO *p_cfg);
...
static void hidd_l2cif_data_ind(UINT16 l2cap_cid,
                                BT_HDR *p_msg);
...
tHID_STATUS hidd_conn_reg(void)
{
	...
	{
		memcpy(&hd_cb.l2cap_ctrl_cfg.qos, &hd_cb.qos_info.ctrl_ch,
		       sizeof hd_cb.l2cap_ctrl_cfg.qos);

		hd_cb.l2cap_ctrl_cfg.qos_present = TRUE;

		memcpy(&hd_cb.l2cap_int_cfg.qos, &hd_cb.qos_info.int_ch,
		       sizeof hd_cb.l2cap_int_cfg.qos);
                                   ...
                                   UINT16 psm, UINT8 l2cap_id)
{
	...
	HIDD_TRACE(EVENT, "HID - Rcvd L2CAP conn ind, PSM: 0x%04x  CID 0x%x", psm,
	           l2cap_cid);

	if (hd_cb.virtual_cable && hd_cb.host_known
	    && memcmp(hd_cb.host_addr, bd_addr, BD_ADDR_LEN) != 0)
	{
		bAccept = FALSE;
	}
	else if (psm == 19)
	...
	if (psm == 17)
	{
		...
		btm_sec_mx_access_request(bd_addr, 17, FALSE, 7,
		                          hd_cb.sec_mask == 0 ? 2 : 1,
		                          &hidd_sec_check_complete_term, NULL);
		return;
	}
...
static void hidd_l2cif_connect_cfm(UINT16 l2cap_cid, UINT16 result)
{
	...
	if ((p_hcon->ctrl_cid != l2cap_cid && p_hcon->intr_cid != l2cap_cid)
	    ...
	    || (l2cap_cid == p_hcon->intr_cid
	        && p_hcon->conn_state != HID_CONN_STATE_CONNECTING_INTR))
	{
		HIDD_TRACE(WARNING, "HID - Rcvd unexpected conn cnf, CID 0x%x ",
		           l2cap_cid);
		return;
	}
	...
	if (l2cap_cid == p_hcon->ctrl_cid)
	{
		...
		btm_sec_mx_access_request(hd_cb.host_addr, 17, TRUE, 7,
		                          hd_cb.sec_mask == 0 ? 2 : 1,
	...
	HIDD_TRACE(EVENT, "HID - got CTRL conn cnf, sent cfg req, CID: 0x%x",
	           l2cap_cid);
}
...
static void hidd_l2cif_config_ind(UINT16 l2cap_cid, tL2CAP_CFG_INFO *p_cfg)
{
	...
	if (p_hcon->ctrl_cid != l2cap_cid && p_hcon->intr_cid != l2cap_cid)
	{
		HIDD_TRACE(WARNING, "HID - Rcvd L2CAP cfg ind, unknown CID: 0x%x",
		           l2cap_cid);
		return;
	}
...
static void hidd_l2cif_config_cfm(UINT16 l2cap_cid, tL2CAP_CFG_INFO *p_cfg)
{
	...
	HIDD_TRACE(EVENT, "HID - Rcvd cfg cfm, CID: 0x%x  Result: %d", l2cap_cid,
	           p_cfg->result);
...
static void hidd_l2cif_disconnect_ind(UINT16 l2cap_cid, BOOLEAN ack_needed)
{
	...
	if (p_hcon->ctrl_cid != l2cap_cid && p_hcon->intr_cid != l2cap_cid)
	{
		HIDD_TRACE(WARNING, "HID - Rcvd L2CAP disc, unknown CID: 0x%x",
		           l2cap_cid);
		return;
	}
...
static void hidd_l2cif_disconnect_cfm(UINT16 l2cap_cid, UINT16 result)
{
	...
	if (p_hcon->ctrl_cid != l2cap_cid && p_hcon->intr_cid != l2cap_cid)
	{
		HIDD_TRACE(WARNING, "HID - Rcvd L2CAP disc cfm, unknown CID: 0x%x",
		           l2cap_cid);
		return;
	}
...
static void hidd_l2cif_cong_ind(UINT16 l2cap_cid, BOOLEAN congested)
{
	...
	HIDD_TRACE(EVENT, "HID - Rcvd L2CAP congestion status, CID: 0x%x  Cong: %d",
	           l2cap_cid, congested);
	...
	(*hd_cb.callback)(14, p_hcon->ctrl_cid == l2cap_cid,
	                  (tHID_DEV_REG_INFO_CBACK_DATA *)&congested);
}
...
static void hidd_l2cif_data_ind(UINT16 l2cap_cid, BT_HDR *p_msg)
{
	...
	if (p_hcon->ctrl_cid != l2cap_cid && p_hcon->intr_cid != l2cap_cid)
	{
		HIDD_TRACE(WARNING, "HID - Rcvd L2CAP data, unknown CID: 0x%x",
		           l2cap_cid);
		GKI_freebuf(p_msg);
		return;
	}
...
tHID_STATUS hidd_conn_snd_data(tSND_DATA_PARAMS *p_data)
{
	...
	do
	{
		...
		*p_out++ = (p_data->trans_type << 4) | (p_data->param & 0xf);
		...
		if (seg_req)
		{
			memcpy(p_out, (UINT8 *)(p_data->buf + 1) + p_data->buf->offset,
			       bytes_copied);
