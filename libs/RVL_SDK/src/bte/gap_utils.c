...
static void btm_cback(UINT16 index, void *p_data)
{
	...
	case GAP_EVT_INQUIRY_COMPLETE:
		...
		inq_cmpl.status =
			(((tBTM_INQUIRY_CMPL *)p_data)->status == BTM_SUCCESS)
				? BT_PASS
				: GAP_ERR_PROCESSING;
	...
	case GAP_EVT_DISCOVERY_COMPLETE:
		...
		{
			GAP_TRACE(EVENT,
				"   GAP Discovery Complete Event(SDP Result: 0x%04x)",
				*(UINT16 *)p_data);
		}
		else
		{
	...
	case GAP_EVT_REM_NAME_COMPLETE:
		((tGAP_REMOTE_DEV_NAME *)p_data)->status = gap_convert_btm_status(
			((tBTM_REMOTE_DEV_NAME *)p_data)->status);

		GAP_TRACE(EVENT,
			"   GAP Remote Name Complete Event (status 0x%04x)",
...
void gap_inq_results_cb(tBTM_INQ_RESULTS *p_results)
{
	...
	GAP_TRACE(
		EVENT,
		"GAP Inquiry Results Callback (bdaddr [%02x%02x%02x%02x%02x%02x])",
	...
	GAP_TRACE(
		...
		p_results->dev_class[0], p_results->dev_class[1],
		p_results->dev_class[2], p_results->clock_offset);

	/* explicitly post-increment */
	for (index = 0, p_cb = &gap_cb.blk[0]; index < GAP_MAX_BLOCKS;
	     index++, p_cb++)
	{
...
void gap_find_addr_name_cb(tBTM_REMOTE_DEV_NAME *p)
{
	...
	{
		GAP_TRACE(EVENT, "   GAP: FindAddrByName Rem Name Cmpl Evt (Status "
		                 "0x%04x, Name [%s])",
		...
		{
			memcpy(p_result->bd_addr,
			       p_cb->p_cur_inq->results.remote_bd_addr, BD_ADDR_LEN);
		...
		else
		{
			if ((p_cb->p_cur_inq = BTM_InqDbNext(p_cb->p_cur_inq)) != NULL)
			{
				if (BTM_ReadRemoteDeviceName(
						p_cb->p_cur_inq->results.remote_bd_addr,
						(tBTM_CMPL_CB *)&gap_find_addr_name_cb)
				    == BTM_CMD_STARTED)
				{
					return;
				}
				...
				{
					p_result->status =
						gap_convert_btm_status((tBTM_STATUS)p->status);
				}
			}
			else
			{
...
void gap_find_addr_inq_cb(tBTM_INQUIRY_CMPL *p)
{
	...
	if (p->status == BTM_SUCCESS)
	{
		if ((p_result->status = gap_find_local_addr_by_name(p_result->devname,
		                                                    p_result->bd_addr))
		    == GAP_NO_DATA_AVAIL)
		{
