                            ...
                            UINT16 num_attr, UINT16 *p_attr_list)
{
	...
	if (!p_db || len < sizeof *p_db || num_attr > SDP_MAX_ATTR_FILTERS
		     ...
		     p_sattr = p_sattr->p_next_attr)
		{
			    ...
			    && p_sattr->attr_value.v.u16 == layer_uuid)
			{
				     ...
				     p_sattr = p_sattr->p_next_attr)
				{
					...
					{
						p_elem->params[p_elem->num_params++] =
							p_sattr->attr_value.v.u16;
					}
					else
					{
						p_elem->params[p_elem->num_params++] =
							p_sattr->attr_value.v.u8;
					}
                      ...
                      UINT32 len, tSDP_DISC_CMPL_CB *p_cb)
{
	...
	if (SDP_InitDiscoveryDb(p_db, len, num_uuids, &init_uuid, 0, NULL)
	    && SDP_ServiceSearchRequest(remote_device, p_db, p_cb))
	{
		result = SDP_SUCCESS;
	}
...
UINT8 SDP_GetNumDiRecords(tSDP_DISCOVERY_DB *p_db)
{
	...
	do
	{
		p_curr_record = SDP_FindServiceInDb(
			p_db, UUID_SERVCLASS_PNP_INFORMATION, p_curr_record);
                       ...
                       tSDP_DISCOVERY_DB *p_db)
{
	...
	if (result == SDP_SUCCESS)
	{
		...
		p_curr_attr =
			SDP_FindAttributeInRec(p_curr_record, ATTR_ID_CLIENT_EXE_URL);
		if (p_curr_attr)
		{
			BCM_STRNCPY_S(p_device_info->rec.client_executable_url,
			              sizeof p_device_info->rec.client_executable_url,
			              (char *)p_curr_attr->attr_value.v.array,
			              SDP_MAX_ATTR_LEN);
		}
		else
		{
		...
		if (p_curr_attr)
		{
			BCM_STRNCPY_S(p_device_info->rec.service_description,
			              sizeof p_device_info->rec.service_description,
			              (char *)p_curr_attr->attr_value.v.array,
			              SDP_MAX_ATTR_LEN);
		}
		else
		...
		if (p_curr_attr)
		{
			BCM_STRNCPY_S(p_device_info->rec.documentation_url,
			              sizeof p_device_info->rec.documentation_url,
			              (char *)p_curr_attr->attr_value.v.array,
			              SDP_MAX_ATTR_LEN);
		}
		else
		...
		if (p_curr_attr)
		{
			p_device_info->rec.primary_record =
...
UINT16 SDP_SetLocalDiRecord(tSDP_DI_RECORD *p_device_info, UINT32 *p_handle)
{
	...
	*p_handle = handle;
		...
		if (p_device_info->client_executable_url[0] != '\0')
		{
			if (strlen(p_device_info->client_executable_url) + 1
			    ...
			    || !SDP_AddAttribute(
					...
					strlen(p_device_info->client_executable_url) + 1,
		...
		if (p_device_info->service_description[0] != '\0')
		{
			if (strlen(p_device_info->service_description) + 1
			    ...
			    || !SDP_AddAttribute(
					...
					strlen(p_device_info->service_description) + 1,
		...
		if (p_device_info->documentation_url[0] != '\0')
		{
			if (strlen(p_device_info->documentation_url) + 1
			      ...
			      || !SDP_AddAttribute(
					  ...
					  strlen(p_device_info->documentation_url) + 1,
	...
	if (result == SDP_SUCCESS)
	{
		if (!SDP_AddAttribute(handle, ATTR_ID_PRIMARY_RECORD, BOOLEAN_DESC_TYPE,
		                      1, &p_device_info->primary_record))
		{
			result = SDP_DI_REG_FAILED;
		}
	}
	...
	if (result == SDP_SUCCESS)
	{
		...
		if (!SDP_AddAttribute(handle, ATTR_ID_VENDOR_ID_SOURCE, UINT_DESC_TYPE,
		                       sizeof p_device_info->vendor_id_source,
		                       temp_u16))
		{
			result = SDP_DI_REG_FAILED;
		}
...
UINT16 SDP_GetLocalDiRecord(tSDP_DI_GET_RECORD *p_device_info, UINT32 *p_handle)
{
	...
	if ((p_rec = sdp_db_find_record(*p_handle)) != NULL)
	{
		    ...
		    != NULL)
		{
			...
			BE_STREAM_TO_ARRAY(p_temp, p_device_info->rec.client_executable_url,
			                   templen);
		}
		    ...
		    != NULL)
		{
			...
			BE_STREAM_TO_ARRAY(p_temp, p_device_info->rec.service_description,
			                   templen);
		}
		    ...
		    != NULL)
		{
			...
			BE_STREAM_TO_ARRAY(p_temp, p_device_info->rec.documentation_url,
			                   templen);
		}
	}
	else
