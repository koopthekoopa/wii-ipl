	...
	for (; p_rec < p_end; ++p_rec)
	{
		for (yy = 0; yy < p_seq->num_uids; ++yy)
		{
			...
			for (xx = 0; xx < p_rec->num_attributes; xx++, p_attr++)
			{
				...
				{
					if (sdpu_compare_uuid_arrays(
							p_attr->value_ptr, p_attr->len,
                                        ...
                                        UINT16 end_attr)
{
	...
	/* explicitly post-increment */
	for (xx = 0, p_at = p_rec->attribute; xx < p_rec->num_attributes;
	     xx++, p_at++)
	{
...
UINT32 SDP_CreateRecord(void)
{
	...
	if (p_db->num_records < SDP_MAX_RECORDS)
	{
		memset(&p_db->record[p_db->num_records], 0,
		       sizeof p_db->record[p_db->num_records]);
                        ...
                        UINT8 *type, UINT8 *len, UINT8 **p_val)
{
	...
	for (xx = 0; xx < num_elem; ++xx)
	{
		...
		if (p - buff > SDP_MAX_ATTR_LEN)
		{
			...
			if (p_head == buff)
			{
				SDP_TRACE(
					ERROR,
					"SDP_AddSequence - too long(attribute is not added)!!");
			...
			{
				SDP_TRACE(ERROR,
				          "SDP_AddSequence - too long, add %d elements of %d",
				          xx, num_elem);
			}
	...
	return SDP_AddAttribute(handle, attr_id, DATA_ELE_SEQ_DESC_TYPE, p - buff,
	                        buff);
}
                            ...
                            UINT16 *p_uuids)
{
	...
	for (xx = 0; xx < num_uuids; xx++, p_uuids++)
	{
		...
		{
			SDP_TRACE(WARNING,
			          "SDP_AddUuidSequence - too long, add %d uuids of %d", xx,
			          num_uuids);
                            ...
                            tSDP_PROTOCOL_ELEM *p_elem_list)
{
	...
	return SDP_AddAttribute(handle, ATTR_ID_PROTOCOL_DESC_LIST,
	                         DATA_ELE_SEQ_DESC_TYPE, offset, buff);
}
                                  ...
                                  tSDP_PROTO_LIST_ELEM *p_proto_list)
{
	...
	return SDP_AddAttribute(handle, ATTR_ID_ADDITION_PROTO_DESC_LISTS,
	                        DATA_ELE_SEQ_DESC_TYPE, p - buff, buff);
}
...
BOOLEAN SDP_DeleteAttribute(UINT32 handle, UINT16 attr_id)
{
	...
	for (xx = 0; xx < sdp_cb.server_db.num_records; xx++, p_rec++)
	{
		if (p_rec->record_handle == handle)
		{
			...
			SDP_TRACE(API,"Deleting attr_id 0x%04x for handle 0x%x", attr_id,
			               handle);
			...
			for (xx = 0; xx < p_rec->num_attributes; xx++, p_attr++)
			{
				if (p_attr->id == attr_id)
				{
					...
					if (len)
					{
						xx = p_rec->free_pad_ptr
						      - (pad_ptr + len - p_rec->attr_pad);
