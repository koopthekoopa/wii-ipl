...
static UINT8 const sdp_base_uuid[LEN_UUID_128] =
{
	0x00, 0x00, 0x00, 0x00,
	0x00, 0x00,
	0x10, 0x00,
                             ...
                             UINT16 error_code, char *p_error_text)
{
	...
	SDP_TRACE(WARNING, "SDP - sdpu_build_n_send_error  code: 0x%x  CID: 0x%x",
                                 ...
                                 UINT16 len2)
{
	...
	else if (len1 > len2)
	{
		if (len1 == 4)
		{
			return p_uuid1[0] == 0x00 && p_uuid1[1] == 0x00
	...
	else
	{
		if (len2 == 4)
		{
			return p_uuid2[0] == 0x00 && p_uuid2[1] == 0x00
...
BOOLEAN sdpu_compare_uuid_with_attr(tBT_UUID *p_btuuid, tSDP_DISC_ATTR *p_attr)
{
	...
	{
		return memcmp(p_btuuid->uu.uuid128, p_attr->attr_value.v.array, 16)
		    == 0;
	}
}
