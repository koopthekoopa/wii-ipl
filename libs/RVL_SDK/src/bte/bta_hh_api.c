...
static tBTA_SYS_REG const bta_hh_reg =
{
	&bta_hh_hdl_event,
	&BTA_HhDisable
};
...
{
	bta_hh_snd_write_dev(dev_handle, HID_TRANS_SET_PROTOCOL, (UINT8)p_type, 0,
	                     0, NULL);
}
...
{
	bta_hh_snd_write_dev(dev_handle, HID_TRANS_CONTROL, (UINT8)c_type, 0, 0,
	                     NULL);
}

void BTA_HhSendData(UINT8 dev_handle, BT_HDR *p_data)
{
	bta_hh_snd_write_dev(dev_handle, HID_TRANS_DATA, BTA_HH_RPTT_OUTPUT, 0, 0,
	                     p_data);
}
