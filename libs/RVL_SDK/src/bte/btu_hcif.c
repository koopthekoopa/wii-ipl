...
static void btu_hcif_hdl_command_complete(UINT16 opcode, UINT8 *p,
                                          UINT16 evt_len);
static void btu_hcif_command_complete_evt(UINT8 *p, UINT16 evt_len);
static void btu_hcif_hdl_command_status(UINT16 opcode, UINT8 status,
                                        UINT8 *p_cmd);
...
static void btu_hcif_store_cmd(BT_HDR *p_buf)
{
	...
	if ((p_cmd =
	         HCI_GET_CMD_BUF(p_buf->len + p_buf->offset - HCIC_PREAMBLE_SIZE))
	    == NULL)
	{
		return;
	}
	...
	btu_start_timer(&btu_cb.cmd_cmpl_timer, BTU_TTYPE_BTU_CMD_CMPL,
...
static void btu_hcif_command_status_evt(UINT8 *p, UINT16 evt_len)
{
	    ...
	    && opcode != HCI_COMMAND_NONE)
	{
		if ((p_cmd = GKI_dequeue(&btu_cb.cmd_cmpl_q)) != NULL)
		{
			...
			{
				p_data = NULL;
				BT_TRACE(HCI, WARNING, "Event mismatch opcode=%X cmd opcode=%X",
				         opcode, cmd_opcode);
			}
		}
...
void btu_hcif_cmd_timeout(void)
{
	...
	BT_TRACE(HCI, WARNING, "BTU HCI command timeout - cmd opcode = 0x%02x",
	         opcode);
