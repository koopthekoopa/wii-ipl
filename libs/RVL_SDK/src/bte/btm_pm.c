...
static tBTM_PM_MODE btm_pm_get_set_mode(UINT8 pm_id, tBTM_PM_MCB *p_cb,
                                        tBTM_PM_PWR_MD *p_mode,
                                        tBTM_PM_PWR_MD *p_res);
static tBTM_STATUS btm_pm_snd_md_req(UINT8 pm_id, int link_ind,
                                     tBTM_PM_PWR_MD *p_mode);
                             ...
                             tBTM_PM_PWR_MD *p_mode)
{
	...
	{
		if (mode == BTM_PM_MD_ACTIVE
		    || (p_mode->max >= p_cb->interval && p_mode->min <= p_cb->interval))
		{
			return BTM_SUCCESS;
		}
	}
                                            ...
                                            tBTM_PM_PWR_MD *p_res)
{
	...
	res = (p_md1->mode - BTM_PM_MD_HOLD) * BTM_PM_NUM_SET_MODES
	    + (p_md2->mode - BTM_PM_MD_HOLD);
                                        ...
                                        tBTM_PM_PWR_MD *p_res)
{
	...
	else if (pm_id == BTM_PM_SET_ONLY_ID
                                     ...
                                     tBTM_PM_PWR_MD *p_mode)
{
	...
	case BTM_PM_MD_ACTIVE:
		...
		case BTM_PM_MD_SNIFF:
			btsnd_hcic_exit_sniff_mode(p_buf,
			                           btm_cb.acl_db[link_ind].hci_handle);
			break;

		case BTM_PM_MD_PARK:
			btsnd_hcic_exit_park_mode(p_buf,
			                          btm_cb.acl_db[link_ind].hci_handle);
			break;
	...
	case BTM_PM_MD_HOLD:
		btsnd_hcic_hold_mode(p_buf, btm_cb.acl_db[link_ind].hci_handle,
		                     md_res.max, md_res.min);
		break;

	case BTM_PM_MD_SNIFF:
		btsnd_hcic_sniff_mode(p_buf, btm_cb.acl_db[link_ind].hci_handle,
		                      md_res.max, md_res.min, md_res.attempt,
		                      md_res.timeout);
		break;

	case BTM_PM_MD_PARK:
		btsnd_hcic_park_mode(p_buf, btm_cb.acl_db[link_ind].hci_handle,
		                     md_res.max, md_res.min);
		break;
...
void btm_pm_proc_cmd_status(tHCI_STATUS status)
{
	    ...
	    && btm_cb.pm_reg_db[btm_cb.pm_pend_id].mask & BTM_PM_REG_NOTIF)
	{
		(*btm_cb.pm_reg_db[btm_cb.pm_pend_id].cback)(
			btm_cb.acl_db[btm_cb.pm_pend_link].remote_addr, pm_status, 0,
			status);
	}
...
void btm_pm_proc_mode_change(tHCI_STATUS hci_status, tHCI_HANDLE hci_handle,
                             UINT8 mode, UINT16 interval)
{
	...
	for (yy = 0; yy < BTM_MAX_PM_RECORDS; ++yy)
	{
		...
		{
			(*btm_cb.pm_reg_db[yy].cback)(p->remote_addr, mode, interval,
			                              hci_status);
		}
	}
	...
	if (btm_cb.acl_db[xx].switch_role_state == BTM_ACL_SWKEY_STATE_MODE_CHANGE)
	{
		    ...
		    && p_dev_rec->sec_flags & BTM_SEC_ENCRYPTED)
		{
			...
			{
				btm_cb.acl_db[xx].switch_role_state =
					BTM_ACL_SWKEY_STATE_ENCRYPTION_OFF;

				return;
			}
		}
		else if (btsnd_hcic_switch_role(btm_cb.acl_db[xx].remote_addr,
		                                !btm_cb.acl_db[xx].link_role))
		{
		...
		if (btm_cb.devcb.p_switch_role_cb)
		{
			...
			(*btm_cb.devcb.p_switch_role_cb)(
				&btm_cb.devcb.switch_role_ref_data.hci_status);
