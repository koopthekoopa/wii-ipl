...
unsigned long strtoul(const char* str, char** end, int base) {
    ...
    value = __strtoul(base, INT_MAX, &__StringRead, &isc, &count, &negative, &overflow);
...
unsigned long long strtoull(const char* str, char** end, int base) {
    ...
    value = __strtoull(base, INT_MAX, &__StringRead, &isc, &count, &negative, &overflow);
...
long strtol(const char* str, char** end, int base) {
    ...
    uvalue = __strtoul(base, INT_MAX, &__StringRead, &isc, &count, &negative, &overflow);

    if (end) {
        *end = (char*)str + count;
    }

    if (overflow || (!negative && uvalue > LONG_MAX) || (negative && uvalue > -(unsigned long)LONG_MIN)) {
        svalue = (negative ? -(unsigned long)LONG_MIN : LONG_MAX);
        errno = ERANGE;
    } else {
