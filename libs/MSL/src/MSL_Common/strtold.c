...
#define success(scan_state) (scan_state & (leading_sig_zeroes | int_digit_loop | frac_digit_loop | exp_digit_loop | leading_exp_zeroes | finished))
    ...
    while (count <= max_width && c != -1 && !final_state(scan_state)) {
        switch (scan_state) {
            ...
            case infin_state: {
                ...
                if (i == 3 || i == 8) {
                    if (sig_negative) {
                        result = (long double)(float)(double)-INFINITY;
                    } else {
                        result = INFINITY;
                    }
            ...
            case hex_state: {
                switch (hex_scan_state) {
                    case hex_start: {
                        ...
                        chptr = mantissa;
                        mantissa_digits = (TARGET_FLOAT_MANT_DIG + 3) / 4;
                        intdigits = 0;
                        nibble_index = 0;
...
double strtod(const char* str, char** endptr) {
    ...
    isc.spNextChar = (char*)str;
    isc.nullCharDetected = 0;

    result = __strtold(INT_MAX, &__StringRead, &isc, &count, &overflow);

    if (endptr) {
        *endptr = (char*)str + count;
    }

    {
        ...
        if (overflow || (d != 0.0 && (a < DBL_MIN || a > DBL_MAX))) {
            errno = ERANGE;
        }
    }
