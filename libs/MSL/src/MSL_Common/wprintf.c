...
// Zero constant for float comparisons; referenced through a volatile pointer
// to prevent MWCC from pooling a duplicate in .sdata2.
const double lbl_81695A68 = 0.0;

static const wchar_t* parse_format(const wchar_t* format_string, va_list* arg, wprint_format* format) {
    wprint_format f;
...
static wchar_t* long2str(long num, wchar_t* buff, wprint_format format) {
...
static wchar_t* longlong2str(long long num, wchar_t* pBuf, wprint_format fmt) {
...
static wchar_t* double2hex(long double num, wchar_t* buff, wprint_format format) {
    wchar_t* p;
    unsigned char* q;
    ...
    if (fabs(num) == *(volatile double*)&lbl_81695A68) {
        p = buff - 6;
        wcscpy(p, L"0x0p0");
        return p;
    }
...
static void round_decimal(decimal* dec, int new_length) {
    ...
    dec->sig.length = (unsigned char)new_length;
}

static wchar_t* float2str(long double num, wchar_t* wbuff, wprint_format format) {
    decimal dec;
    decform form;
    wchar_t* pw;
    char* p;
    ...
    switch (*dec.sig.text) {
        ...
        case 'I': {
            if (num < *(volatile double*)&lbl_81695A68) {
                pw = wbuff - 5;
...
static int __wpformatter(void* (*WriteProc)(void*, const wchar_t*, size_t), void* WriteProcArg, const wchar_t* format_str, va_list arg) {
    ...
    _ep = (wprint_format*)_fw;
    format_ptr = format_str;
    chars_written = 0;

    while (*format_ptr) {
        if (!(curr_format = wcschr((wchar_t*)format_ptr, L'%'))) {
        ...
        format_ptr = parse_format(format_ptr, (va_list*)arg, (wprint_format*)_fw);

        switch (_ep->conversion_char) {
            case L'd':
            case L'i': {
                if (_ep->argument_options == long_argument) {
                    long_num = va_arg(arg, long);
                } else if (_ep->argument_options == long_long_argument) {
                    long_long_num = va_arg(arg, long long);
                } else if (_ep->argument_options == intmax_argument) {
                    long_long_num = va_arg(arg, intmax_t);
                } else if (_ep->argument_options == size_t_argument) {
                    long_num = va_arg(arg, size_t);
                } else if (_ep->argument_options == ptrdiff_argument) {
                    long_num = va_arg(arg, ptrdiff_t);
                } else {
                    long_num = va_arg(arg, int);
                }

                if (_ep->argument_options == short_argument) {
                    long_num = (short)long_num;
                }

                if ((_ep->argument_options == long_long_argument) || (_ep->argument_options == intmax_argument)) {
                    if (!(buff_ptr = longlong2str(long_long_num, buff + 512, *_ep))) {
                        goto conversion_error;
                    }
                } else {
                    if (!(buff_ptr = long2str(long_num, buff + 512, *_ep))) {
                        goto conversion_error;
                    }
                }
            ...
            case L'X': {
                if (_ep->argument_options == long_argument) {
                    ...
                    long_long_num = va_arg(arg, unsigned long long);
                } else if (_ep->argument_options == intmax_argument) {
                    long_long_num = va_arg(arg, intmax_t);
                } else if (_ep->argument_options == size_t_argument) {
                    long_num = va_arg(arg, size_t);
            ...
            case L'g':
            case L'G': {
                if (_ep->argument_options == long_double_argument) {
                    long_double_num = va_arg(arg, long double);
                } else {
                    long_double_num = va_arg(arg, double);
                }

                if (!(buff_ptr = float2str(long_double_num, buff + 512, *_ep))) {
                    goto conversion_error;
                }
            ...
            case 'A': {
                ...
                if (!(buff_ptr = double2hex(long_double_num, buff + 512, *_ep))) {
                    goto conversion_error;
                }
            ...
            case L's': {
                if (_ep->argument_options == wchar_argument) {
                    ...
                    if (_ep->alternate_form) {
                        num_chars = (unsigned char)*buff_ptr++;

                        if (_ep->precision_specified && num_chars > _ep->precision) {
                            num_chars = _ep->precision;
                        }
                    } else if (_ep->precision_specified) {
                        num_chars = _ep->precision;

                        if ((string_end = wmemchr(buff_ptr, 0, num_chars)) != NULL) {
                    ...
                    if (_ep->alternate_form) {
                        ...
                        if ((cstring_end = (char*)memchr(strptr, 0, num_chars)) != NULL) {
        ...
        if (_ep->justification_options != left_justification) {
            fill_char = (_ep->justification_options == zero_fill) ? L'0' : L' ';
            ...
            while (field_width < _ep->field_width) {
...
void* __wStringWrite(void* pCtrl, const wchar_t* pBuffer, size_t char_num) {
    __wOutStrCtrl* ctrl = pCtrl;
    size_t chars = (ctrl->charsWritten + char_num <= ctrl->maxCharCount) ? char_num : ctrl->maxCharCount - ctrl->charsWritten;
    void* result = wmemcpy(ctrl->spwCharStr + ctrl->charsWritten, pBuffer, chars);
...
int swprintf(wchar_t* s, size_t n, const wchar_t* format, ...) {
    ...
    wosc.charsWritten = 0;
    count = __wpformatter(&__wStringWrite, &wosc, format, args);
