...
static char* double2hex(long double num, char* buff, print_format format) {
    ...
    if (fabs(num) == *(volatile double*)&lbl_81695A00) {
        p = buff - 6;
        strcpy(p, "0x0p0");
        return p;
    }
...
static char* float2str(long double num, char* buff, print_format format) {
    ...
    switch (*dec.sig.text) {
        ...
        case 'I': {
            if (num < *(volatile double*)&lbl_81695A00) {
                p = buff - 5;
...
int __pformatter(void* (*WriteProc)(void*, const char*, size_t), void* WriteProcArg, const char* format_str, va_list arg) {
    ...
    while (*format_ptr) {
        ...
        format_ptr = parse_format(format_ptr, (va_list*)arg, (print_format*)_fw);
        ...
        switch (_ep->conversion_char) {
            ...
            case 'i': {
                ...
                if (_ep->argument_options == char_argument) {
            ...
            case 's': {
                ...
                if (_ep->alternate_form) {
                    ...
                    if ((string_end = (char*)memchr(buff_ptr, 0, num_chars)) != NULL) {
        ...
        if (_ep->justification_options != left_justification) {
            fill_char = _ep->justification_options == zero_fill ? '0' : ' ';
...
int fprintf(FILE* pFile, const char* format, ...) {
...

    {
        va_list args;
        va_start(args, format);
        res = __pformatter(&__FileWrite, pFile, format, args);
        return res;
    }
}
...
int vprintf(const char* pFormat, va_list arg) {
    ...
    ret = __pformatter(&__FileWrite, stdout, pFormat, arg);
    return ret;
}
