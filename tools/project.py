...
class ProjectConfig:
    def __init__(self) -> None:
        ...
        self.bstool_tag: Optional[str] = None  # Git tag
        self.bstool_path: Optional[Path] = None  # If None, download
        ...
        self.wad_path: Optional[Path] = None  # Path to IPL wad
        self.app_name: Optional[Path] = None  # IPL executable filename
        self.app_path: Optional[Path] = None  # Full path to IPL executable
        self.dol_path: Optional[Path] = None  # Path to the DTK usable DOL file
...
def generate_build_ninja(
    ...
    if config.bstool_path:
        bstool = config.bstool_path
    elif config.bstool_tag:
        bstool = build_tools_path / f"bstool{EXE}"
        n.build(
            outputs=bstool,
            rule="download_tool",
            implicit=download_tool,
            variables={
                "tool": "bstool",
                "tag": config.bstool_tag,
            },
        )
    else:
        sys.exit("ProjectConfig.bstool_tag missing")
    ...
    n.build(
        outputs="tools",
        rule="phony",
        inputs=[dtk, sjiswrap, bstool, wrapper, compilers, binutils, objdiff],
    )
    n.newline()
    ...
    n.comment("Generate BootStage")
    n.rule(
        name="elf2bs",
        command=f"{bstool} convert --base-file $base_app --in-file $in --out-file $out",
        description="BOOTSTAGE $out",
    )
    n.newline()
    ...
    class LinkStep:
        ...
        def output(self) -> Path:
            if self.module_id == 0:
                return build_path / f"{self.name}.app"
            else:
    ...
    if build_config:
        ...
        def add_unit(build_obj, link_step: LinkStep):
            ...
            # For Wii Menu's BSS (is there a better way?)
            if obj_name == "GARBAGE_DATA":
                return
        ...
        n.build(
            ...
            variables={"base_app": config.app_path},
        )
    ...
    # Extract BootStage
    ###
    n.comment("Extract BootStage")
    n.rule(
        name="extract_exe",
        command=f"{dtk} vfs cp $in:$target_idx $out",
        description="EXTRACT $out",
    )
    n.build(
        inputs=config.wad_path,
        outputs=config.app_path,
        rule="extract_exe",
        implicit=dtk,
        variables={"target_idx": config.app_name},
    )
    n.newline()
    ...
    # Convert BootStage
    ###
    n.comment("Convert BootStage")
    n.rule(
        name="convert_exe",
        command=f"{bstool} dtk --in-file $in --out-file $out",
        description="CONVERT $out",
    )
    n.build(
        inputs=config.app_path,
        outputs=config.dol_path,
        rule="convert_exe",
        implicit=[
            bstool,
            config.app_path,
    ...
    n.comment("Split BS2 into relocatable objects")
    n.rule(
        name="split",
...
def generate_objdiff_config(
    ...
    def add_unit(
        ...
        obj_path, obj_name = build_obj["object"], build_obj["name"]
        # For Wii Menu's BSS (is there a better way to do this?)
        if obj_name == "GARBAGE_DATA":
            return
...
def generate_compile_commands(
    ...
    def add_unit(build_obj: Dict[str, Any]) -> None:
        # For Wii Menu's BSS (is there a better way to do this?)
        if build_obj["name"] == "GARBAGE_DATA":
            return
